#!/usr/bin/env python3
"""B4/WP9 fallback: train-interpolated first-token proxy unit tests (wscllm).

Covers the B3_W 60s gate-2 failure fallback implemented in
workload/llama2_7b_inference/metrics_postprocess.py:

  1. authoritative W_bytes / KV-bytes-per-token conversions (frozen
     wsc_llm_scheduler formulas (the wscllm twin of face_scheduler) fed by trace_config.csv -- no invented values);
...
class ModelBytesTests(unittest.TestCase):
    def test_authoritative_conversions(self):
        ...
        numbers (test_wsc_llm_scheduler.py
        test_llama2_7b_tp6_partition_is_exact_without_model_padding pins
        the same value)."""
...
class ProxyFormulaTests(unittest.TestCase):
    ...
    def test_dl1_pinned_to_completion(self):
        ...
        wscllm 0902 full: 5/268 dl1 rows, up to 0.56s early)."""
        index = _fake_index(
            {"r": (0, _ledger_row(tick=1_000_000, iterations=8))},
            {0: 9_000_000},
        )
...
class EndToEndRowsTests(unittest.TestCase):
    ...
    def test_dl1_row_pinned_and_hard_invariant_spares_exact(self):
        """e2e dl1 (2026-09-05): a decode_length==1 debut riding an N=8
        train is PINNED to completion (the share<1 interpolation used to
        land before the completion tick), the dl1 hard invariant holds
        (proxy row first_token_ns == completion_ns), and an EXACT dl1 row
        with first_token < completion stays legal (WP9_CONTRACT §6
        informational TP skew -- the invariant must not fire on exact)."""
        manifest_path = (Path(self.records[0]["manifest_path"]).parent
                         / "manifest.json")
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        manifest["requests"][0]["decode_length"] = 1
        manifest["requests"][0]["final_context_tokens"] = 4097
        manifest["requests"][1]["decode_length"] = 1
        manifest["requests"][1]["final_context_tokens"] = 2049
        ...
        with (self.results_dir / "train_ledger.jsonl").open(
            ...
            sink.write(json.dumps(_ledger_row(
                train_id="batch_train_i0_2", tick=9_000_000)) + "\n")
        rows = self._rows()
        # queue 0: no exact value -> dl1 proxy == completion (pinned).
        row = rows["session_0_request_0"]
        self.assertEqual(row["first_token_ns"], 41_000_000)
        self.assertEqual(row["first_token_ns"], row["completion_ns"])
        self.assertEqual(row["first_token_source"], "train_interpolated")
        self.assertIn("pinned_to_completion", row["instructions"])
        # queue 1: exact dl1, first_token(2.5M) < completion(42M) -- legal
        # per WP9_CONTRACT §6, must not trip the hard invariant.
        exact = rows["session_1_request_0"]
        self.assertEqual(exact["first_token_ns"], 2_500_000)
        self.assertEqual(exact["first_token_source"], "exact")
