...
class OnlineSchedulerBase:
    ...
    def __init__(
        ...
        *,
        ...
        # 产出时即完整,提供 sink 时逐行流式写出(缺省 None = 行即弃,无
        # 驻留缓冲;生产路径 online_service 恒提供 sink)。online_stats
        ...
        # 无关;单批只经索引/队列直接访问,不存在 O(总规模) 全量扫描)。
        # 行: {delivery_sequence, tick, scanned_entries}。
        self.profile_rows = []
        ...
        #   每已应用交付一行 {delivery_sequence, tick, reasons,
        #     scheduler_self_ns, node_count} 经 online_stats_sink 流式写出
        #     (缺省 None = 行即弃,无驻留缓冲)。
        ...
        #     是 unseen/arrival 校验逻辑漂移,属结构性断言。空 manifest
        #     (expected_request_count == 0,如测试只读 probe / 0-request
        #     运行)取哨兵 1:tracker 契约要求上限为正整数,而该情形
        #     in_flight 的合法上界 0 已由 _process_arrivals 的 unseen
        #     fail-closed 保证(任何到达先 raise,到不了 tail 增长点),
        #     哨兵不放松任何检查;非空 manifest 上限仍为请求数(精确上界);
        ...
        self.propagating_tail.register(
            ...
            lambda: self.in_flight, max(1, self.expected_request_count))
        self.propagating_tail.register(
    ...
    def build_graph_batch(self) -> dict:
        ...
        # 完整,提供 profile_sink 时逐行流式写出;缺省缓冲在本列表)。
        profile_row = {
    ...
    def _record_sensing_query(self, delta: dict) -> None:
        ...
        # B3:流式落盘(行在产出时即完整;缺省无 sink 行即弃,无驻留缓冲)。
    ...
    def _start_batch(self, delta: dict) -> None:
        self._profile_batch = {"scanned_entries": 0}
        self._batch = {
            "delivery_sequence": delta["delivery_sequence"],
            "tick": delta["tick"],
            "reasons": list(delta["reasons"]),
