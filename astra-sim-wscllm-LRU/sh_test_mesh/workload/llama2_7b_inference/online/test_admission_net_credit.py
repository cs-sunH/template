...
def _scheduler(kv_manager):
    ...
    # B3(2026-09-06):发射账本镜像 no-op 桩(见 _try_admit_prefill 接线)。
...
class NetCreditStubTests(unittest.TestCase):
    def test_full_fail_net_retry_success_backfills(self):
        ...
        kv_manager = _StubKVManager(
            ...
            snapshots=[
                SimpleNamespace(location=LOCAL_HBM, instance_index=0,
                                shard_bytes=credit,
                                local_shard_bytes=credit),
    ...
    def test_snapshot_off_decode_target_keeps_single_full_reserve(self):
        ...
        for snapshot in (
            SimpleNamespace(location=LOCAL_HBM, instance_index=1,
                            shard_bytes=(200_000_000,),
                            local_shard_bytes=(200_000_000,)),  # 别的实例
...
def _seed_turn_one(manager):
    ...
    assert snapshot is not None and snapshot.location == LOCAL_HBM
