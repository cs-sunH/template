...
class Topology {
  public:
    ...
    [[nodiscard]] std::shared_ptr<const FluidRoute>
    fluid_route(DeviceId src, DeviceId dest) const noexcept;
  ...
  protected:
    ...
    std::vector<std::shared_ptr<const Link>> directed_links;

    mutable std::unordered_map<uint64_t, std::shared_ptr<const FluidRoute>> fluid_route_cache;
