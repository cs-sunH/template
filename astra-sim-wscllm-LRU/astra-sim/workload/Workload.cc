...
void Workload::issue_send_comm(const ExecutionDriven::NodeView& node) {
    ...
    if (sys->hbm_bandwidth_contention &&
        local_hbm_bandwidth_model != nullptr && size > 0 &&
...
void Workload::call(EventType event, CallData* data) {
    ...
    if (event == EventType::CollectiveCommunicationFinished) {
        ...
        if (data == nullptr) {
            ...
            // 中-4①: joined nodes complete with the deterministic network
            // event kept in the join entry -- the later-arriving HBM-side
            // General callback must not skip the p2p bandwidth stat.
            finish_generic_node(
                wlhd->node_id,
