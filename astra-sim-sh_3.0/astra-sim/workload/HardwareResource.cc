...
void HardwareResource::occupy(const ExecutionDriven::NodeView& node) {
    ...
    // sh_3.0 fourth slot (static semantic preserved): a MEM node carrying
    // is_local_hbm_kv_restore occupies the hbm_dma slot; a remote MEM node
    // falls through to the comm slot below (static path behavior).
    if ((node.kind == ExecutionDriven::NodeKind::MemLoad ||
