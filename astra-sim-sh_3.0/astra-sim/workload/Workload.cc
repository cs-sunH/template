...
void Workload::issue(shared_ptr<Chakra::FeederV3::ETFeederNode> node) {
    ...
    if (MetricCollector::instance().enabled()) {
        MetricCollector::instance().on_node_issue(sys->id, node->id(),
                                                  Sys::boostedTick());
    }
    ...
    if (sys->replay_only) {
        ...
        if ((node->type() == ChakraNodeType::MEM_LOAD_NODE) ||
            ...
            if (node->get_attr<bool>("is_local_hbm_kv_restore", false)) {
...
void Workload::issue_local_hbm_kv_restore(
    ...
    const uint64_t tensor_size = node->tensor_size<uint64_t>();
...
void Workload::issue_comp(shared_ptr<Chakra::FeederV3::ETFeederNode> node) {
    ...
    if (MetricCollector::instance().enabled()) {
        MetricCollector::instance().on_compute_issue(
            sys->id, node->num_ops<uint64_t>(), node->tensor_size<uint64_t>());
    }
    ...
    if (node->has_attr("remote_weight_bytes")) {
        ...
        double remote_weight_bytes =
            static_cast<double>(node->remote_weight_bytes<uint64_t>());
    ...
    if (local_hbm_bandwidth_model != nullptr) {
        local_hbm_bandwidth_model->issue_compute(
            node->num_ops<uint64_t>(), node->tensor_size<uint64_t>(), wlhd);
    } else {
        uint64_t runtime =
...
void Workload::skip_invalid(shared_ptr<Chakra::FeederV3::ETFeederNode> node) {
    ...
    if (MetricCollector::instance().enabled()) {
        MetricCollector::instance().on_node_complete(sys->id, node->id(),
                                                     Sys::boostedTick());
    }
    if (this->sys->track_local_mem) {
