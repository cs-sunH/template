...
void MetricCollector::finalize(const std::vector<Sys*>& systems,
    ...
    for (const Sys* sys : sorted_systems) {
        ...
        // (hbm_kv_restore_bandwidth_sharing / hbm-bandwidth-contention),
        ...
        if (hbm_model != nullptr) {
            ...
            hbm_record["model"] =
                "LocalHbmBandwidthModel: N-way strict equal split -- every "
                "active HBM job on the rank (inference COMP, KV-restore DMA, "
                "NoC p2p comm endpoints COMM_READ/COMM_WRITE, pool "
                "endpoints POOL_READ/POOL_WRITE) receives full_rate/N of "
                "the configured local-mem-bw; any completion immediately "
                "re-splits among the survivors. Reads and writes share one "
                "aggregate bandwidth scalar (no peak/sustained "
                "distinction). When hbm-bandwidth-contention is off, no "
                "comm/pool jobs exist and the model degenerates to the "
                "legacy two-user (COMP + restore) 50/50 sharing.";
            ...
            hbm_record["hbm_shared_definition"] =
                ...
                "the rank HBM bandwidth under the N-way equal split "
                "(full_rate/N)";
