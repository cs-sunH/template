...
class NetworkParser {
  public:
    ...
    [[nodiscard]] uint64_t get_fluid_max_active_flows() const noexcept;

    [[nodiscard]] uint64_t get_fluid_max_route_memberships() const noexcept;

    [[nodiscard]] uint64_t get_progress_report_event_interval() const noexcept;
  ...
  private:
    ...
    uint64_t fluid_max_active_flows;

    uint64_t fluid_max_route_memberships;
     ...
     *    which can be "Ring", "FullyConnected", "Switch", "Mesh", or "Line"
