# astra-sim-sh_3.0 使用说明（Execution-Driven 在线仿真仓库）
...
- **策略语义（保留对象，未改动）**：三段式准入：首请求避边缘 / LOCAL·PARTIAL sticky / REMOTE 全集负载均衡；decode 固定 prefill 同实例；三态 KV；sidecar_restore
