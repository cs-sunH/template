#!/usr/bin/env bash
# sh_3.0 phase-1 step-1-10 官方在线 runner:replay 模式(决策日志回放,LUT 时钟)。
...
RUN_DIR=$1
REQUEST_CSV=${2:?"request_csv 必填(request-neutral:请按方案文档 sh_3.0仓库改造详细执行方案.md §3 步骤 0-1 物化输入后显式传入)"}
DECISION_LOG=${3:?"decision_log 必填(request-neutral:请按方案文档 sh_3.0仓库改造详细执行方案.md §3 步骤 0-1 物化决策日志后显式传入)"}
