...
class StreamingTraceOutputs:
    """Write rank ET nodes incrementally and publish files only on success."""
    ...
    def __init__(
        ...
        *,
        ...
        self._temporary_paths = {
            rank: path.with_suffix(f"{path.suffix}.tmp")
            for rank, path in self._final_paths.items()
        }
        self._outputs: dict[int, BinaryIO] = {}
        self._finalized = False
        try:
            for rank in range(config.npus_count):
                temporary_path = self._temporary_paths[rank]
                output = temporary_path.open("wb")
                self._outputs[rank] = output
                encode_message(output, _build_metadata(config, plan, rank=rank))
        except BaseException:
            self.abort()
            raise

    def write_node(self, rank: int, node: object) -> None:
        encode_message(self._outputs[rank], node)
...
class ParallelTraceOutputs:
    """Replay rank-local trace commands in bounded CPU worker processes.

    The coordinator remains responsible for FACE's global ordering, then sends
    immutable builder calls to workers that own disjoint rank files.  ET files
    stay in a per-run staging directory until every worker reports the expected
    node count, so a failed worker never publishes a partial trace.
    """
...
def _planner_cache_path(config: FaceTraceConfig) -> Path:
    cache_key = hashlib.sha1(
        (
            f"{PLANNER_CACHE_VERSION}|{config.configuration_digest}|"
            f"{config.trace_granularity}|{len(config.request_queue)}"
        ).encode("utf-8")
    ).hexdigest()[:20]
    return (
        SH_TEST_DIR
        / "generated"
        / "planner_cache"
        / f"face_plan_{cache_key}.pickle"
    )


def load_or_build_face_plan(config: FaceTraceConfig) -> FacePlan:
    cache_path = _planner_cache_path(config)
    if cache_path.is_file():
        with cache_path.open("rb") as source:
            plan = pickle.load(source)
        if (
            not isinstance(plan, FacePlan)
            or len(plan.requests) != len(config.request_queue)
            or plan.iterations_recorded
            != (config.trace_granularity == "token_expanded")
        ):
            raise RuntimeError(f"invalid FACE planner cache: {cache_path}")
        print(f"[sh_test] Reusing FACE planner cache: {cache_path}", file=sys.stderr)
        return plan
    ...
    try:
        with temporary_path.open("wb") as target:
            pickle.dump(plan, target, protocol=pickle.HIGHEST_PROTOCOL)
        temporary_path.replace(cache_path)
    finally:
        if temporary_path.exists():
            temporary_path.unlink()
    print(f"[sh_test] Saved FACE planner cache: {cache_path}", file=sys.stderr)
    return plan
...
def build_trace_label(
    ...
    if plan is None:
        ...
        prefill_work_tokens = tuple(
            request.prefill_context_tokens - request.history_tokens_before
            for request in plan.requests
        )
    prefill_label = range_label(prefill_work_tokens)
    ...
    return (
        ...
        f"hbm{hbm_gib}_kvsuffixhalf_r{config.kv_reserve_context_tokens}_"
...
def _task_load_snapshot_dict(
    snapshot: InstanceTaskLoadSnapshot,
) -> dict[str, object]:
    return {
        "instance_index": snapshot.instance_index,
        "running_prefill_task_load_ns": (
            snapshot.running_prefill_task_load_ns
        ),
...
def _session_snapshot_dict(snapshot: SessionKVSnapshot) -> dict[str, object]:
    return {
        ...
        "resident_prefix_layers": snapshot.resident_prefix_layers,
        "local_bytes": snapshot.local_bytes,
        "remote_bytes": snapshot.remote_bytes,
        "local_shard_bytes": list(snapshot.local_shard_bytes),
        "remote_shard_bytes": list(snapshot.remote_shard_bytes),
    ...
    *,
    ...
    metadata.attr.extend(
        [
            ...
            ChakraAttr(
                name="hardware_case",
                string_val=str(config.hardware_metadata["slug"]),
            ),
            ...
            ChakraAttr(
                name="kv_eviction_policy",
                string_val="fifo_half_layer_suffix_then_full_fallback",
            ),
            ChakraAttr(
                name="kv_partial_resident_prefix_layers",
                uint64_val=config.layers - config.layers // 2,
            ),
            ChakraAttr(
                name="hbm_kv_restore_bandwidth_sharing",
                string_val="50_50_while_overlapped_then_100_to_survivor",
            ),
        ]
    )
    return metadata
...
def _candidate_dict(candidate: DecodeCandidateCost) -> dict[str, object]:
    return {
        ...
        "remaining_hbm_capacity_bytes": (
            candidate.remaining_hbm_capacity_bytes
        ),
...
def _request_plan_dict(
    ...
    *,
    ...
    prefill_tokens_processed = (
        request_plan.prefill_context_tokens - request_plan.history_tokens_before
    )
    return {
        ...
        "prefill_length": request.prefill_length,
        "source_prefix_tokens": request.prefix_tokens,
        "source_input_tokens_total": request.input_tokens_total,
        ...
        "trace_representation": {
            ...
            "prefill_chunk_count_represented": (
                prefill_tokens_processed + plan.p_chunk - 1
            )
            // plan.p_chunk,
        ...
        "prefill_assignment": {
            ...
            "load_key_before_assignment": list(request_plan.prefill_assignment_key),
            "instance_task_loads_before_assignment": [
                {
                    **_task_load_snapshot_dict(snapshot),
                    "hbm_feasible": (
                        request_plan.prefill_hbm_feasible_instances[
                            snapshot.instance_index
                        ]
                    ),
                }
                for snapshot in request_plan.prefill_instance_loads
            ],
            "policy": (
                "resident_prefix_instance_affinity"
                if request_plan.prefill_affinity_reason is not None
                else (
                    "per_rank_hbm_feasible_then_minimum_roofline_remaining_"
                    "task_load_then_oldest_arrival_then_config_order"
                )
            ),
            "affinity_reason": request_plan.prefill_affinity_reason,
        },
        "decode_assignment": {
            ...
            "policy": (
                ...
                "max(remaining_hbm_capacity_bytes),config_order"
            ),
        },
...
def write_face_trace(
    ...
    *,
    ...
    if not parallel_output:
        trace_outputs = StreamingTraceOutputs(
            config=config,
            plan=plan,
        ...
        builders = {
            rank: TraceBuilder(
                ...
                node_sink=(
                    lambda node, rank=rank: trace_outputs.write_node(rank, node)
                ),
                retain_nodes=False,
            )
            for rank in range(config.npus_count)
    ...
    deferred_session_locations: dict[str, str] = {}

    def mark_pending_history_store(transfer: KVTransfer) -> None:
        ...
        if pending_request_id is None:
            deferred_session_locations[session_id] = location
            return
        pending_history[pending_request_id].location = location
    ...
    for request_plan in ordered_plans:
        ...
        reconcile_pending_history_location(pending_gate, request_plan)
        transfers_by_stage: dict[str, list[dict[str, object]]] = {
            "history_evictions": [],
            "history_transfer": [],
            "prefill_evictions": [],
        ...
        def emit_transfer(
            ...
            *,
            ...
            # Doc sec.4.3 code 7: transfer-arrival memory anchors are only
            # meaningful for transfers that deliver KV to their target ranks
            # (history restore/migration, Prefill-to-Decode migration).
            anchor_sink = None
            if metrics is not None and stage in (
                "history_transfer",
                "prefill_decode_transfer",
            ):
                anchor_sink = lambda rank, node_id: metrics.add_event(  # noqa: E731
                    rank,
                    node_id,
                    EVENT_MEMORY_ANCHOR_COMPLETE,
            ...
            record = _emit_kv_transfer(
                ...
                pending_gate=gate,
                trigger_gate=trigger_gate,
                transfer_anchor_sink=anchor_sink,
            )
            ...
            action_sequence += 1
            if transfer.kind == "remote_store":
                mark_pending_history_store(transfer)
            return record
        ...
        suffix_restore_nodes_by_rank: dict[int, int] = {}
        suffix_ready_nodes_by_rank: dict[int, int] = {}
        ...
        if partial_history_restore:
            ...
            history_transfer = request_plan.history_transfer
            history_before = request_plan.history_location_before
            assert history_before is not None
            if (
                history_transfer.kind != "remote_load"
            ...
            readiness_barriers["prefill_prefix"] = _emit_tp_readiness_barrier(
                builders=builders,
                group=prefill_group,
            ...
            if set(suffix_restore_nodes_by_rank) != set(prefill_group.ranks):
                raise RuntimeError(
                    ...
                    f"request={request_plan.request_id}, "
                    f"session={request_plan.session_id}, "
                    f"history_tokens={request_plan.history_tokens_before}, "
                    f"expected={list(prefill_group.ranks)}, "
                    f"observed={sorted(suffix_restore_nodes_by_rank)}, "
                    f"transfer_shards={len(history_transfer.shards)}"
                )
            ...
            readiness_barriers["prefill_suffix"] = {
                **suffix_readiness,
                "layer_start": history_transfer.layer_start,
                "layer_end": history_transfer.layer_end,
                "restore_node_ids_by_rank": [
                    [rank, suffix_restore_nodes_by_rank[rank]]
                    for rank in prefill_group.ranks
                ],
                "policy": (
                    "resident prefix compute overlaps remote suffix restore; "
                    "first suffix operator waits for every TP rank's target "
                    "HBM write through a tagged point-to-point readiness barrier"
                ),
            }
        else:
        ...
        # Metrics stage boundaries (doc sec.6): the first real prefill node
        # per rank is the next node the emission below produces on that rank;
        # the artificial readiness/timer gate nodes are already behind us.
        prefill_first_node_by_rank: dict[int, int] = {}
        prefill_last_node_by_rank: dict[int, int] = {}
        if metrics is not None:
            for rank in prefill_group.ranks:
                prefill_first_node_by_rank[rank] = builders[rank].next_id
        if config.trace_granularity == "token_expanded":
            ...
            while processed < prefill_tokens_to_process:
                ...
                for relative_rank, rank in enumerate(prefill_group.ranks):
                    ...
                    if partial_history_restore and chunk_index == 0:
                        assert request_plan.history_location_before is not None
                        suffix_start = (
                        ...
                        transformer_pass(
                            builders[rank],
                            phase=(
                                f"{prefix}_prefill_chunk{chunk_index:04d}_prefix"
                            ),
                            layer_start=0,
                        ...
                        transformer_pass(
                            builders[rank],
                            phase=(
                                f"{prefix}_prefill_chunk{chunk_index:04d}_suffix"
                            ),
                    ...
                    if metrics is not None:
                        # Doc sec.6.3: the last *real* prefill node, saved
                        # before the artificial one-byte end barrier.
            ...
            for relative_rank, rank in enumerate(prefill_group.ranks):
                ...
                if partial_history_restore:
                    assert request_plan.history_location_before is not None
                    suffix_start = (
                        request_plan.history_location_before.resident_prefix_layers
                    )
                    transformer_pass_aggregated(
                        builders[rank],
                ...
                if metrics is not None:
                    # Doc sec.6.3: the last *real* prefill node, saved before
                    # the artificial aggregated end barrier.
                    prefill_last_node_by_rank[rank] = builders[rank].previous_id
                builders[rank].all_reduce(
        ...
        if metrics is not None:
            for rank in prefill_group.ranks:
                metrics.add_event(
                    rank,
                    prefill_first_node_by_rank[rank],
                    EVENT_PREFILL_START,
                    request_plan.queue_index,
                )
                metrics.add_event(
                    rank,
                    prefill_last_node_by_rank[rank],
                    EVENT_PREFILL_END,
                    request_plan.queue_index,
                )
        ...
        # Metrics stage boundaries (doc sec.6): first real decode node per
        # rank; the decode readiness barrier nodes are already behind us.
        decode_first_node_by_rank: dict[int, int] = {}
        decode_last_node_by_rank: dict[int, int] = {}
        if metrics is not None:
            for rank in decode_group.ranks:
                decode_first_node_by_rank[rank] = builders[rank].next_id
        if config.trace_granularity == "token_expanded":
        ...
        if metrics is not None:
            for rank in decode_group.ranks:
                metrics.add_event(
                    rank,
                    decode_first_node_by_rank[rank],
                    EVENT_DECODE_START,
                    request_plan.queue_index,
                )
                metrics.add_event(
                    rank,
                    decode_last_node_by_rank[rank],
                    EVENT_DECODE_END,
                    request_plan.queue_index,
                )
        ...
        if following is not None:
            ...
            if request_plan.session_id in deferred_session_locations:
                completion_location = deferred_session_locations.pop(
            ...
            deferred_session_locations.pop(request_plan.session_id, None)
    ...
    nodes_per_rank = {
        str(rank): builders[rank].node_count for rank in range(config.npus_count)
    }
    if not parallel_output:
        trace_outputs.finalize()
    else:
        trace_outputs.finalize(nodes_per_rank)
    request_records.sort(key=lambda record: int(record["queue_index"]))
    if metrics is not None:
        # Sidecar metrics manifest (doc sec.4): written only after the .et
        # files are finalized so the trace digest covers the published bytes.
        metrics.write_manifest(
            output_dir=output_dir,
            config=config,
            plan=plan,
            node_count_by_rank={
                int(rank): count for rank, count in nodes_per_rank.items()
            },
            et_paths_by_rank={
                rank: output_dir / f"{config.output_prefix}.{rank}.et"
                for rank in range(config.npus_count)
            ...
            kv_digest_payload=kv_event_payload_sh2(request_records),
        )
    manifest = {
        ...
        "mapping_strategy": (
            "Roofline remaining-task-load Prefill ordering; weighted Instance_map "
            "bounded by D2D/DRAM; LUT minimum per-die incremental decode latency; "
            "node-private HBM with FIFO half-layer-suffix offload, full-session "
            "fallback, and pipelined suffix restore"
        ),
        ...
        "prefill_assignment_policy": {
            "primary_key": "ascending_total_task_load_ns",
            "tie_break": (
                "oldest_last_arrival_ns_with_never_used_first_then_trace_config_order"
            ),
            "task_load_unit": "roofline_estimated_service_time_ns",
            "components": [
                "running_prefill_remaining_task_load",
                "queued_prefill_chunk_task_load",
                "active_decode_remaining_task_load",
            ],
            "first_request_candidate_set": (
                "instances_without_mesh_boundary_ranks; "
                "falls_back_to_all_instances_when_no_edge_free_instance_exists"
            ),
            "resident_history": (
                "sticky_to_kv_resident_instance_for_local_or_partial_hbm"
            ),
            "remote_history": "load_balance_over_all_instances_including_boundary",
            "history_aware": (
                "Prefill uses prior-session KV plus processed prompt context; "
                "Decode uses its current full context"
            ),
            "average_decode_length_tokens": plan.average_decode_length,
            "average_decode_length_source": (
                "arithmetic mean over all source request_queue_csv rows before "
                "session selection"
            ),
            "decode_cutoff": (
                "zero remaining load once generated tokens reach or exceed the mean"
            ),
        },
        "decode_policy": {
            "placement": "same_instance_as_prefill",
            "candidate_evaluation": "none",
            "kv_migration": "none_local_hit_only",
        },
        "lut": {
            "path": str(lut_path),
        ...
        "request_queue_context_csv": (
            None
            if config.request_queue_context_csv is None
            else str(config.request_queue_context_csv)
        ),
        ...
        "source_average_decode_length": config.source_average_decode_length,
        ...
        "planning_iteration_recording_policy": (
            "full_records_for_token_expanded; omitted_for_request_aggregated"
        ),
        "static_et_request_emission_order": (
            "kv_causality_topological_then_prefill_start_then_queue_index"
        ),
        "kv_management": {
            "policy": "fifo_half_layer_suffix_then_full_session_fallback",
            "admission_control": (
                "final_kv_per_rank_capacity_reservation_with_event_driven_wait_retry"
            ),
            "session_history": (
                "cumulative_KV_tracks resident prefix layers and remote suffix"
            ),
            "suffix_layers": config.layers // 2,
            "resident_prefix_layers_after_stage_one": (
                config.layers - config.layers // 2
            ),
            "layer_count_source": "model_config_not_hard_coded",
            "active_session_evictable": False,
            "stage_one": (
                "FIFO full-local sessions; offload [ceil(L/2), L)"
            ),
            "stage_two": (
                "after every inactive full-local session is halved, FIFO full "
                "eviction stores each session's remaining resident prefix"
            ),
            "partial_restore": (
                "retain instance affinity; execute resident prefix while remote "
                "suffix loads; synchronize every target HBM write before the "
                "first suffix operator"
            ),
            "hbm_bandwidth_overlap": (
                "per NPU, inference and KV restore each receive 50 percent while "
                "both have outstanding HBM bytes; survivor immediately receives "
                "100 percent"
            ),
            ...
            "transfer_order_per_request": [
                ...
                "history_transfer",
                "prefill_evictions",
                "prefill_prefix_ready_or_full_kv_ready_barrier",
                "overlapped_suffix_restore_and_prefix_compute_when_partial",
                "prefill",
                "decode_evictions",
                "prefill_decode_transfer",
                "decode_tp_kv_ready_barrier",
...
def print_shell_config(
    ...
    session_count = len({request.session_id for request in config.request_queue})
    prefill_work_tokens = (
        derive_prefill_work_tokens(config.request_queue)
        if plan is None
        else tuple(
            request.prefill_context_tokens - request.history_tokens_before
            for request in plan.requests
    ...
    assignments = {
        ...
        "PREFILL_CHUNK_SIZE": (
            PREFILL_CHUNK_SIZE if plan is None else plan.p_chunk
        ),
        "AVERAGE_DECODE_LENGTH": (
            config.source_average_decode_length
            if plan is None
            else plan.average_decode_length
        ),
        "TRACE_GRANULARITY": config.trace_granularity,
        "PREFILL_RANGE": range_label(prefill_work_tokens),
...
def _parse_jobs(value: str, source: str) -> int:
    try:
        ...
        raise SystemExit(f"{source} must be a positive integer, got {value!r}") from error
    if jobs < 1:
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    if metrics is None:
        ...
        # stream, so a metrics-enabled run always rebuilds the deterministic
        # plan under the read-only observer instead of reusing the planner
        # cache (cache files are left untouched for metrics-off runs).  The
        # streaming planner-LUT iteration aggregates (doc sec.8.8) are
        # observed during the same planning pass and emitted only after a
        # successful trace write.
