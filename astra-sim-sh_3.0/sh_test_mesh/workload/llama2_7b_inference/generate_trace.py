...
) -> tuple[dict[str, str], tuple[InferenceGroup, ...]]:
    """Parse the shared config/inference-group CSV structure once."""
...
class TraceBuilder:
    ...
    def arm_dependency(self, node_id: Optional[int]) -> None:
        """Attach an already-emitted node to the next node on this rank."""

        if node_id is not None:
            self.pending_extra_dependencies.append(node_id)

    def chain_checkpoint(self) -> tuple[Optional[int], tuple[int, ...]]:
        """Capture dependency-chain state before emitting a parallel branch."""

        return self.previous_id, tuple(self.pending_extra_dependencies)

    def restore_chain(
        self,
        checkpoint: tuple[Optional[int], tuple[int, ...]],
    ) -> None:
        """Return to a checkpoint without removing already-emitted nodes."""
    ...
    def mem_store(
        ...
        tensor_size: int,
        *,
        hbm_access_mode: int = 0,
    ) -> None:
        node = self._new_node(name, MEM_STORE_NODE)
        node.attr.append(self._uint64_attr("tensor_size", tensor_size))
        # N-way HBM contention pool endpoint charge (0 none / 1 read /
        # 2 write). Only non-default values emit the attribute so every
    ...
    def local_hbm_kv_restore(self, name: str, tensor_size: int) -> None:
        """Emit a target-HBM DMA write that may overlap inference compute."""

        node = self._new_node(name, MEM_LOAD_NODE)
        node.attr.extend([
            self._uint64_attr("tensor_size", tensor_size),
    ...
    def comm_send(
        ...
        *,
        ...
        # N-way HBM contention comm endpoint charge (default true). Only
        # the non-default (pass-through) value emits the attribute so every
        # legacy trace stays byte-identical.
        if not hbm_charge:
...
class TraceCommandRecorder:
    """Record replayable ``TraceBuilder`` calls without building protobuf nodes.

    FACE scheduling and communication-tag allocation must remain globally
    ordered, but each rank's Chakra nodes can be constructed independently
    after that order is fixed.  This lightweight mirror preserves the builder
    IDs and dependency-chain state in the coordinator process, then emits the
    public builder calls for a CPU worker to replay.
    """
    ...
    def arm_dependency(self, node_id: Optional[int]) -> None:
        ...
        if node_id is None:
            return
        self.pending_extra_dependencies.append(node_id)
        self._record("arm_dependency", node_id)
    ...
    def restore_chain(
        ...
        self.previous_id, dependencies = checkpoint
        self.pending_extra_dependencies = list(dependencies)
        self._record("restore_chain", checkpoint)
    ...
    def mem_store(
        ...
        *,
        ...
        self._new_node()
        self._record(
            "mem_store", name, tensor_size, hbm_access_mode=hbm_access_mode
        )
    ...
    def mem_load(
        ...
        *,
        ...
        self._new_node()
        self._record(
            "mem_load", name, tensor_size, hbm_access_mode=hbm_access_mode
        )

    def local_hbm_kv_restore(self, name: str, tensor_size: int) -> None:
        self._new_node()
        self._record("local_hbm_kv_restore", name, tensor_size)
...
def transformer_pass(
    ...
    *,
    ...
    if layer_end is None:
        layer_end = layers
    if not 0 <= layer_start < layer_end <= layers:
        raise ValueError("Transformer layer range is outside the model")
...
def transformer_pass_aggregated(
    ...
    *,
    ...
    if not 0 <= layer_start < layer_end <= layers:
        raise ValueError("Transformer layer range is outside the model")
    active_layers = layer_end - layer_start
    layer_label = (
        "all_layers"
        if layer_start == 0 and layer_end == layers
        else f"layers{layer_start:02d}_{layer_end - 1:02d}"
    )
    ...
    def aggregate_comp(category_name: str) -> None:
        ...
        builder.comp(
            f"{phase}_{layer_label}_{category_name}",
            ops * active_layers,
            tensor_size * active_layers,
            remote_read * active_layers,
        )
    ...
    builder.all_reduce(
        f"{phase}_{layer_label}_attention_all_reduce",
        attention_collective_bytes * active_layers,
        pg_name,
    )
    ...
    builder.all_reduce(
        f"{phase}_{layer_label}_mlp_all_reduce",
        mlp_collective_bytes * active_layers,
        pg_name,
    )
