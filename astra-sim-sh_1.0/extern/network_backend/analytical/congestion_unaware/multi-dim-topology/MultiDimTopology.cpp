...
EventTime MultiDimTopology::send(const DeviceId src, const DeviceId dest, const ChunkSize chunk_size) const noexcept {
    ...
    auto comms_delay = EventTime(0);
    auto has_transfer = false;

    // Dimension-order routing without link contention. This mirrors the
    // congestion-aware MultiDimTopology route order, but keeps the original
    // congestion-unaware analytical delay model inside each dimension.
    for (auto dim = 0; dim < dims_count; dim++) {
        ...
        comms_delay += topology->send(src_local_id, dest_local_id, chunk_size);
