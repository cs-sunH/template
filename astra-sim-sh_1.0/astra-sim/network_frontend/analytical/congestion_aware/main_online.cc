...
main_online.cc -- online (execution-driven) entry point (sh_1.0 port; blueprint wscllm phase 1).
...
struct OnlineDriverContext {
    ...
    // Phase-4 sensing (方案 §6.2 操作 2): remote-FIFO 实账本层 per-epoch
    // snapshot sidecar handle (<bridge_dir>/remote_fifo_ledger.jsonl; one
    // row per delivery epoch). Opened in main only when --sensing-enabled,
    // so ordinary runs carry zero overhead; closed at run end.
    std::FILE* remote_fifo_log = nullptr;
...
void ed_commit_cb(void* arg) {
    ...
    //      (批命名空间,不挂请求;不回收则 T_max/oracle 模式下 run-end
    //      空注册表审计必红) ----
...
void ed_driver_tick_end(void* ctx) {
    ...
    driver->stats.snapshot_ns +=
        std::chrono::duration_cast<std::chrono::nanoseconds>(
            ...
            .count();
    // Phase-4 sensing (方案 §6.2 操作 2 / R3 方案 §3.6): remote-FIFO ledger
    // snapshot at this delivery epoch, keyed by the REAL backend ports the
    // AnalyticalRemoteMemory resolved (PER_NODE / MEMORY_POOL share a port
    // across ranks; PER_NPU + npu-ids keys by array index). The row now also
    // carries the memory architecture, each port's source ranks and the
    // by_rank issued-attribution view (the reconcile script's RF3 reads it).
    // Sensing-gated sidecar; pure observation, one row per delivery epoch.
    if (driver->sensing_enabled && driver->remote_fifo_log != nullptr) {
        auto& fifo = ExecutionDriven::RemoteFifoLedger::instance();
        // R3 DESIGN §4.5: the row is built by the ledger itself so the
        // production sidecar and the focused fixture share ONE formatter
        // and can never drift apart.
        const std::string row = fifo.sidecar_row(
            driver->delivery_seq,
            driver->event_queue->get_current_time());
        std::fwrite(row.data(), 1, row.size(), driver->remote_fifo_log);
        std::fputc('\n', driver->remote_fifo_log);
    }
...
int main(int argc, char* argv[]) {
    ...
    // R3 (方案 §3.6): label the ledger with the backend's own parsed
    // memory architecture (single source of truth -- main_online must not
    // re-derive the string; the blueprint repos have no sensing run and
    // never reach this call).
    RemoteFifoLedger::instance().set_architecture(
        memory_api->architecture_name());
    ...
    // per-epoch coverage, byte conservation vs the Python decision log and
    // the end drain.
    if (online_cli.sensing_enabled) {
        driver_ctx.remote_fifo_log = std::fopen(
            (online_cli.bridge_dir + "/remote_fifo_ledger.jsonl").c_str(),
            "w");
        if (driver_ctx.remote_fifo_log == nullptr) {
            std::cerr << "[online] cannot open remote-fifo ledger sidecar in "
                      << online_cli.bridge_dir << std::endl;
            return 1;
        }
    }
    ...
    // sh_1.0 §8.1 MEM-node validation: the port-owning ranks (edge ranks)
    // come from the same remote-memory configuration file the
    // AnalyticalRemoteMemory itself parses (red-line backend -- read-only
    // here; we mirror its "npu-ids" list, not the backend). MEM nodes are
    // only committable on these ranks; an empty list (no per-NPU memory
    // expansion) rejects every MEM node (fail-closed).
    {
        std::ifstream remote_config_stream(remote_memory_configuration);
        if (!remote_config_stream) {
            std::cerr << "[online] cannot open remote-memory configuration: "
                      << remote_memory_configuration << std::endl;
            return 1;
        }
        nlohmann::json remote_json;
        remote_config_stream >> remote_json;
        if (remote_json.contains("npu-ids") &&
            remote_json["npu-ids"].is_array()) {
            for (const auto& npu_id : remote_json["npu-ids"]) {
                committer_ctx.remote_mem_ranks.push_back(npu_id.get<int>());
            }
        }
    }
    ...
    while (true) {
        ...
        // Defect fix A (2026-08-16, synced from face-defectfix2-done; 源分析
        // face主动测试错误分析.md 缺陷 A): pump() QUEUES Submit commands into
        // the bounded ingress queue; they only register as pending alarms
        // (on_alarm_scheduled) on the NEXT drain_commands(). Between this
        // pump() and that drain the service counters pass through a vacuum --
        // active==0 && pending_alarm==0 while queued-but-undrained commands
        // exist -- and the finished() check below could end the run with
        // thousands of CSV rows still undelivered (F2/F3/F4: ack_count !=
        // delivery_count, 恒差 1; the window had topped up un-consumed rows
        // right as the last pending alarm of the previous batch fired).
        // Drain whatever pump() just queued BEFORE consulting finished(), so
        // the check can never observe the vacuum. The extra drain is a no-op
        // when pump() queued nothing (the common case).
        ...
        if (event_queue->finished()) {
            ...
            if (mailbox.has_decision_work()) {
                ...
                // Defect fix C (2026-08-16, synced from face-defectfix2-done;
                // 源分析 face主动测试错误分析.md 缺陷 C): the main list is
                // empty but the same-tick deferred queue still holds
                // unexecuted events (scheduled outside a proceed context --
                // e.g. a deferred issue pass appended after the drain pass
                // ended). finished() is blind to them and no proceed() would
                // ever run to drain them: force the explicit next decision
                // boundary (same mechanism as the milestone branch above) so
                // a proceed() executes and drains the deferred queue. NOT a
                // delivery deferral (no mailbox work): deferred_from_tick
                // stays 0.
                driver_ctx.stats.global_wakeup_count += 1;
                event_queue->schedule_event(
                    event_queue->get_current_time() + 1,
                ...
                // at startup), every finished() counter that COULD have a
                // queue event is zeroed or its event is gone, the mailbox is
                ...
                // 50-minute silent stalls (F5/F6). Fail closed with the full
                // counter diagnostics instead, so the dead end is reported
                // immediately and the failure is attributable.
                //
                // Guard convergence (2026-08-16, synced from
                // face-defectfix2-done): this fail-closed REPLACES the old
                // sh_1.0 30-second starvation guard (机制特有改动 (b)) --
    ...
    // Phase-4 sensing (方案 §6.2 操作 2): remote-FIFO 实账本层 run-end
    // report + fail-closed drain check. R3 (方案 §3.6): counters are keyed
    // by the REAL backend ports; issued != completed at run end = a lost
    // completion = mechanism bug. Printed for every online run; the sidecar
    // rows are sensing-gated.
    {
        auto& fifo = ExecutionDriven::RemoteFifoLedger::instance();
        uint64_t peak_pending = 0;
        uint64_t peak_bytes = 0;
        for (const std::size_t port_index : fifo.active_ports()) {
            const auto* port = fifo.port(port_index);
            const uint64_t in_flight =
                port->issued_count - port->completed_count;
            const uint64_t active = in_flight > 0 ? 1u : 0u;
            if (in_flight - active > peak_pending) {
                peak_pending = in_flight - active;
            }
            if (port->peak_in_flight_bytes > peak_bytes) {
                peak_bytes = port->peak_in_flight_bytes;
            }
        }
        std::cout << "[online] remote fifo ledger: ports="
                  << fifo.active_ports().size()
                  << " issued=" << fifo.total_issued_count()
                  << " issued_bytes=" << fifo.total_issued_bytes()
                  << " completed=" << fifo.total_completed_count()
                  << " completed_bytes=" << fifo.total_completed_bytes()
                  << " peak_pending=" << peak_pending
                  << " peak_in_flight_bytes=" << peak_bytes << std::endl;
        // R3 (方案 §3.6): explicit mapping evidence -- memory architecture,
        // real port count, the port-key rule and the busiest port's source
        // rank multiplicity (per-port detail in the sensing sidecar rows).
        // Purely additive: cpp.log is an allowed-diff log and the reconcile
        // regex above matches only the pre-existing line.
        {
            std::size_t ranks_per_port_max = 0;
            for (const std::size_t port_index : fifo.active_ports()) {
                if (fifo.port_ranks(port_index).size() > ranks_per_port_max) {
                    ranks_per_port_max = fifo.port_ranks(port_index).size();
                }
            }
            std::cout << "[online] remote fifo ledger mapping: architecture="
                      << fifo.architecture() << " ports="
                      << fifo.active_ports().size() << " port-key="
                      << memory_api->port_mapping_rule() << " ranks-per-port="
                      << ranks_per_port_max << std::endl;
        }
        if (driver_ctx.remote_fifo_log != nullptr) {
            std::fclose(driver_ctx.remote_fifo_log);
            driver_ctx.remote_fifo_log = nullptr;
        }
        if (!fifo.drained()) {
            std::cerr << "[Error] (main_online) remote-FIFO ledger not "
                         "drained at run end (a port-FIFO completion was "
                         "lost) -- aborting (fail-closed)"
                      << std::endl;
            std::abort();
        }
    }
