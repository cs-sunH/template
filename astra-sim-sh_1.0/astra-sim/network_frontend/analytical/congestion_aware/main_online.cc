...
void ed_driver_tick_end(void* ctx) {
    ...
    driver->stats.snapshot_ns +=
        std::chrono::duration_cast<std::chrono::nanoseconds>(
            ...
            .count();
    // Phase-4 sensing (方案 §6.2 操作 2): remote-FIFO 实账本层 snapshot at
    // this delivery epoch (per-port active/pending/in-flight bytes,
    // reconstructed from the issue/completion counters -- the exact
    // single-server FIFO state; see Workload.hh). Sensing-gated sidecar;
    // pure observation, one row per delivery epoch.
    if (driver->sensing_enabled && driver->remote_fifo_log != nullptr) {
        auto& fifo = ExecutionDriven::RemoteFifoLedger::instance();
        std::fprintf(driver->remote_fifo_log,
                     "{\"delivery_sequence\": %llu, \"tick\": %llu, "
                     "\"ports\": [",
                     (unsigned long long)driver->delivery_seq,
                     (unsigned long long)driver->event_queue
                         ->get_current_time());
        bool first_port = true;
        for (const int rank : fifo.active_ports()) {
            const auto* port = fifo.port(rank);
            ...
            std::fprintf(driver->remote_fifo_log,
                         "%s{\"rank\": %d, \"active\": %llu, "
                         "\"pending\": %llu, \"in_flight_bytes\": %llu, "
                         "\"issued_count\": %llu, \"completed_count\": %llu, "
                         "\"issued_bytes\": %llu, \"completed_bytes\": %llu}",
                         first_port ? "" : ", ", rank,
                         (unsigned long long)active,
                         (unsigned long long)(in_flight - active),
                         (unsigned long long)in_flight_bytes,
                         (unsigned long long)port->issued_count,
                         (unsigned long long)port->completed_count,
                         (unsigned long long)port->issued_bytes,
                         (unsigned long long)port->completed_bytes);
            first_port = false;
        }
        std::fprintf(driver->remote_fifo_log, "]}\n");
    }
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        // the old sh_1.0 30-second starvation guard that used to live here
        // (机制特有改动 (b): event_queue drained && !svc.finished() &&
    ...
    // report + fail-closed drain check. Per-port counters reconstruct the
    // AnalyticalRemoteMemory port FIFO exactly (single-server invariant);
    // issued != completed at run end = a lost completion = mechanism bug.
    // Printed for every online run (replay MEM bypasses the real FIFO by
    // contract ⑦ item ④, so its counters stay zero -- that asymmetry is the
    // contract, not a leak); the sidecar rows are sensing-gated.
    {
        auto& fifo = ExecutionDriven::RemoteFifoLedger::instance();
