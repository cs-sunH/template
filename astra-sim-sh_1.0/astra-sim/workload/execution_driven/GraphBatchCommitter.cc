...
GraphBatchCommitter -- execution-driven mechanism layer (sh_1.0 port; blueprint wscllm phase 5).
...
std::optional<std::string> GraphBatchCommitter::validate(
    ...
    try {
        ...
        for (const auto& node : batch.nodes) {
            ...
            // [sh_1.0 port note] the mother's hunk also rebinds node "mem"
            // here -- this repo's schema carries no "mem" sub-object (the
            // HBM attrs live under compute; see the §8.1 MEM-node pass
            // below), so only "comm" is rebound.
            ...
            // sh_1.0 §8.1 MEM-node validation (types 2/3 = MEM_LOAD /
            // MEM_STORE): tensor_size must be positive (a zero-byte remote
            // access has no execution semantics; the online builder clamps
            // to >= 1) and the node's rank must own an
            // AnalyticalRemoteMemory port (remote_memory.json npu-ids -- the
            // 26 edge ranks). With no mapping configured, ANY MEM node is
            // rejected: it could never execute (fail-closed).
            if (type == 2 || type == 3) {
                const uint64_t tensor_size =
                    compute.value("tensor_size", uint64_t{0});
                if (tensor_size == 0) {
                    return "node[" + std::to_string(node_index) +
                           "] MEM node with non-positive tensor_size";
                }
                if (std::find(ctx_.remote_mem_ranks.begin(),
                              ctx_.remote_mem_ranks.end(),
                              rank) == ctx_.remote_mem_ranks.end()) {
                    return "node[" + std::to_string(node_index) +
                           "] MEM node on rank " + std::to_string(rank) +
                           " without a remote-memory port mapping";
                }
            }
        ...
        // sh_1.0 three-segment adaptation: the coverage rule is one-
        // directional here. Every watch's (request_id, stage) must be
        // covered by this batch's nodes (an uncovered watch is always a
        // bug), but a batch may carry tail nodes whose (request_id, stage)
        // has no new watch -- sh_1.0's third emission segment
        // (completion_evictions + next-turn interval gates) arrives at the
        // REQUEST_COMPLETE boundary while its (request, decode) watch was
        // registered by the second segment's batch. wscllm-style two-segment
        // batches still satisfy the stricter equality (no behavior change
        // for them); the deviation is recorded in sh_1.0改造执行实录.md.
