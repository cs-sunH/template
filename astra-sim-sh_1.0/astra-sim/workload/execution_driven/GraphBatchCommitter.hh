...
class GraphBatchCommitter {
  public:
    ...
    struct Context {
        int num_ranks = 0;
        // sh_1.0 §8.1 MEM-node validation: ranks that own an
        // AnalyticalRemoteMemory port (remote_memory.json "npu-ids" = the
        // 26 edge ranks). MEM_LOAD/MEM_STORE nodes are only legal on these
        // ranks AND with a positive tensor_size; an empty vector = no
        // remote-memory mapping configured and ANY MEM node is rejected
        // (fail-closed: a MEM node can never execute without a port). The
        // official online main always parses and passes the set; fixtures
        // that never emit MEM nodes leave it empty.
        std::vector<int> remote_mem_ranks;
