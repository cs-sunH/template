    ...
    Fixture()
        : ingress(4096),
          committer(GraphBatchCommitter::Context{
              3, {0, 1}, &sources, &registry, &ingress,
...
// sh_1.0 §8.1: MEM_LOAD/MEM_STORE node builder (type 2/3). The fixture
// configures remote_mem_ranks = {0, 1}: rank 2 owns no remote-memory port.
nlohmann::json mem_node(int rank, uint64_t id, int type,
                        uint64_t tensor_size) {
    return {
        {"rank", rank}, {"id", id}, {"type", type},
        {"name", type == 2 ? "remote_load" : "remote_store"},
        {"is_cpu_op", false}, {"is_timer_op", false}, {"inputs_values", ""},
        {"request_id", "r1"}, {"stage", "prefill"}, {"generation", 0},
        {"compute", {{"num_ops", 0}, {"tensor_size", tensor_size},
                     {"runtime_ns", 0}}},
        {"comm", comm_defaults()},
        {"coll", coll_defaults()},
    };
...
void test_negative_cases(Fixture& f) {
    ...
    // sh_1.0 §8.1 MEM-node validation (zero side effects on reject):
    // non-positive tensor_size / rank without a remote-memory port mapping /
    // both rejected; a legal MEM node (port rank + positive tensor_size)
    // passes validate().
    {
        GraphBatch b = base;
        b.nodes[0] = mem_node(0, 0, 2, 0);
        expect_reject(f, delta, b,
                      "B: MEM node with non-positive tensor_size");
    }
    {
        GraphBatch b = base;
        b.nodes[0] = mem_node(0, 0, 3, 4096);
        b.nodes[0]["compute"]["tensor_size"] = 0;
        expect_reject(f, delta, b,
                      "B: MEM_STORE node with tensor_size 0");
    }
    {
        GraphBatch b = base;
        // rank 2 owns no port in the fixture (remote_mem_ranks = {0, 1});
        // replace the rank-2 collective (id 1) so ids stay unique per rank.
        b.nodes[6] = mem_node(2, 1, 2, 8192);
        expect_reject(f, delta, b,
                      "B: MEM node on rank without remote-memory port");
    }
    {
        GraphBatch b = base;
        b.nodes[0] = mem_node(0, 0, 2, 4096);
        expect(!f.committer.validate(delta, b).has_value(),
               "B: legal MEM node on port rank passes validate()");
    }
}
