...
int main() {
    ...
    // 2026-08-16; NOTE pre-existing baseline defect fixed 2026-08-18: this
    // assertion expected the pre-backport 30e9 default while OnlineCli
    // already defaulted to 0, so the tagged baseline test failed)
    assert(parse_ok({"--online-mode", "strategy"}, out));
