...
即删的 has_any_response 判定)。本仓适配点(其余与 face 版逐字一致):
  1. 调度器装配段按本仓 online_service.py:Sh10OnlineScheduler，Decode
     代价由当前硬件、模型和队列状态直接计算 Roofline 估计；
  2. _snapshot 的 KV 观测源(批 F1 修复 2026-08-21):face 版读
     kv_manager.events 长度 + _kv_events_emitted 事件水位,本仓
     KVCacheManager 无事件流水(KV 动作不经 kv_actions 批次通道),
     改用等价状态源——逐会话 session_snapshots() + 逐 rank
     hbm_snapshots()(见 _snapshot 注释);
  3. --config 传参(批 F1 修复 2026-08-21):load_face_trace_config 只收
     Path,argparse 产出的 str 先经 Path() 归一。
"""
...
def _snapshot(scheduler, sink):
    ...
    return {
        ...
        # sh_1.0 适配(批 F1 2026-08-21):face 版此处读 kv 事件水位
        # (kv_manager.events 长度 + _kv_events_emitted);本仓 KVCacheManager
        # 无事件流水,KV 动作不经 kv_actions 批次通道,等价幂等观测源 =
        # 逐会话 KV 状态(位置/实例/上下文/字节/完成时刻/active)+ 逐 rank
        # HBM KV 占用——任何重放误重入账都会改变其中至少一项。
        "kv_sessions": [
            (snapshot.session_id, snapshot.location, snapshot.instance_index,
             snapshot.context_tokens, snapshot.total_bytes,
             snapshot.last_completion_ns, snapshot.active)
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        description="sh_1.0 phase-4 §7.2 idempotency fixture")
