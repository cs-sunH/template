#!/usr/bin/env python3
"""test_first_token_split.py -- WP9 首 token 首步批拆分钉子测试(2026-08-26)。

钉住拆分语义(SH_FIRST_TOKEN_SPLIT，B4 起缺省关；显式 "1" 开——
60s 决策等价门-2 失败后的主规格 §1.6 A 类处置，默认 proxy 口径，
证据 /tmp/slo_wps/b3/S1/ 与 /tmp/slo_wps/gates/B3_S1.FAILED):
  ...
  (b) 批结构:首步批 = joiner 迁移 + 起始标记 + 首迭代体(weight_passes=1)
      + first_token 标记(名含 "first_token",携 debut request_id)+
      唤醒标记(批命名空间),无 drain/exit/哨兵/end barrier;
      余量批 = 余量体(weight_passes=iterations-1)+ 全部标记 + end barrier
      (挂点语义不变);两段权重/ops 字节合计 == 整列发射;
  ...
  (d) decode_length=1 边界:debut 无独立标记,exit 标记改名含
      ...
      (标记增强仍生效);
  (e) OFF 等价:SH_FIRST_TOKEN_SPLIT=0 时发射节点/边与无 first_token
      计划的整列发射逐字节一致;
...
def _train_bytes(builder, train_id, ranks=DECODE_RANKS):
    """列车归属节点(train_id 命名空间)的 compute 字节合计。"""
    total_ops = 0
    total_bytes = 0
    for rank in ranks:
...
def _request(request_id, decode=4):
    return SimpleNamespace(
        request_id=request_id,
        session_id=SESSION,
        turn_index=0,
...
def _runtime(request_id, *, decode=4, consumed=0, ctx=100):
    rt = _OnlineRuntime(
        request=_request(request_id, decode=decode),
        queue_index=0,
...
class SplitEnvironmentTest(unittest.TestCase):
    ...
    def test_default_off_and_explicit_on(self):
        try:
            ...
            # 2026-08-27 B4 缺省翻转:B3_S1 60s 门-2(决策等价)失败 →
...
class DebutPlanTest(unittest.TestCase):
    ...
    def test_debut_is_fresh_joiner_only(self):
        ...
        self.s._runtimes = {REQUEST_B: fresh, REQUEST_A: veteran}
        plan = self._plan()
        token = self.s._first_token_plan(plan, [REQUEST_A, REQUEST_B])
    ...
    def test_no_debut_returns_none(self):
        self.s._runtimes = {REQUEST_A: _runtime(REQUEST_A, consumed=3)}
        self.assertIsNone(self.s._first_token_plan(self._plan(), [REQUEST_A]))

    def test_disabled_returns_none(self):
        self.s._runtimes = {REQUEST_B: _runtime(REQUEST_B, consumed=0)}
        try:
            os.environ["SH_FIRST_TOKEN_SPLIT"] = "0"
            self.assertIsNone(
                self.s._first_token_plan(self._plan(), [REQUEST_B]))
        finally:
            os.environ.pop("SH_FIRST_TOKEN_SPLIT", None)

    def test_tmax_1_plus_7_split(self):
        """(c):8 迭代(T_max=8 截断同构)拆 1+7——首步 = 首chunk + 各成员
        第 1 span;余量 = 7 chunk + 成员剩余 span;总量守恒。"""
        self.s._runtimes = {REQUEST_B: _runtime(REQUEST_B, consumed=0)}
        plan = self._plan(iterations=8)
        token = self.s._first_token_plan(plan, [REQUEST_B])
    ...
    def test_decode_length_1_debut_uses_exit_marker(self):
        ...
        self.s._runtimes = {REQUEST_B: _runtime(REQUEST_B, decode=1)}
        plan = self._plan()
        token = self.s._first_token_plan(plan, [REQUEST_B])
    ...
    def test_single_iteration_enhances_without_split(self):
        ...
        self.s._runtimes = {REQUEST_B: _runtime(REQUEST_B, decode=4)}
...
class SplitEmissionTest(unittest.TestCase):
    ...
    def test_split_batch_structure(self):
        """(b):首步批无标记/barrier,含 first_token+唤醒标记;余量批
        挂全部标记与 end barrier;两段权重/ops 合计 == 整列。"""
        ...
        self.assertEqual(
            self.builder_block_end(builder, REQUEST_A),
    ...
    @staticmethod
    def builder_block_end(builder, request_id):
        return builder._block_ends[request_id]["seg2"][DECODE_RANKS[0]]
    ...
    def test_off_equivalence_byte_identical(self):
        ...
        try:
            os.environ["SH_FIRST_TOKEN_SPLIT"] = "0"
            s = _bare_scheduler()
            s._runtimes = {REQUEST_B: _runtime(REQUEST_B)}
            self.assertIsNone(
