...
#
# B3_S1 60s gate-2 (decision equivalence) failed -- the split's physical
# perturbation (one extra delivery per split train -> tick shifts) is
# amplified by the closed loop (see /tmp/slo_wps/gates/B3_S1.FAILED,
# evidence /tmp/slo_wps/b3/S1/) -- so the default first-token source falls
...
# (one-train-in-flight gate, sh10_online_scheduler.py in_flight_train), so
