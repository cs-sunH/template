...
def filter_tp_degrees(
    tp_degrees: Sequence[int], config: FaceTraceConfig
) -> tuple[int, ...]:
...
def _build_metadata(
    config: FaceTraceConfig, point: BenchPoint, *, rank: int
) -> GlobalMetadata:
...
def write_point_trace(
    config: FaceTraceConfig, point: BenchPoint, point_dir: Path
) -> Path:
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    config = load_face_trace_config(Path(args.service_config))
