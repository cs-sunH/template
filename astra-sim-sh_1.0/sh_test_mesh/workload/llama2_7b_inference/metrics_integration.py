"""Glue between the SH1 ET generator and the frozen metrics schema.
...
planner memory deltas recorded by the ``KVCacheManager`` embedded in
``face_scheduler`` through the read-only :class:`metrics_schema.MemoryMetricsObserver`,
and finally writes the ``metrics_manifest.json`` sidecar next to the generated
``.et`` files.
...
- ``kv_event_digest``: SHA256 of the canonical JSON of the SH1 KV event
  payload: every planned ``KVTransfer`` of every request in queue order (the
  complete-session FIFO remote-store/load and NoC migration event stream)
  plus the final weighted-instance-graph edges and remaining capacities.
"""
...
def _kv_transfer_payload_row(transfer: Any) -> dict[str, Any]:
    """One deterministic row per planned KVTransfer (face_scheduler.KVTransfer)."""
...
def kv_event_payload_sh1(plan: Any) -> dict[str, Any]:
    """SH1 KV event stream: every planned transfer per request, in queue order.

    The SH1 KVCacheManager has no separate event log; its observable KV
    decisions are exactly the per-request transfer tuples plus the final
    weighted-instance-graph state (doc sec.12.4).
    """

    requests: list[dict[str, Any]] = []
    for request_plan in sorted(plan.requests, key=lambda item: item.queue_index):
        stages: dict[str, Any] = {}
        for stage in (
            "history_evictions",
            "history_transfer",
            ...
            transfers = getattr(request_plan, stage)
            if transfers is None:
                stages[stage] = None
            elif isinstance(transfers, tuple):
                stages[stage] = [
                    _kv_transfer_payload_row(transfer) for transfer in transfers
                ]
            else:
                stages[stage] = _kv_transfer_payload_row(transfers)
        requests.append(
            {
                ...
                "kv_location_after_completion": (
                    request_plan.kv_location_after_completion
                ),
...
class ServiceMetrics:
    """Per-generation metrics context for one SH1 service trace run."""
    ...
    def write_manifest(
        ...
        *,
        ...
        if not self.memory.deltas:
            raise RuntimeError(
                "no memory deltas were recorded; the embedded KVCacheManager "
