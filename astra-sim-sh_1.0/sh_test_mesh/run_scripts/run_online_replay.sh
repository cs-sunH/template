#!/usr/bin/env bash
# sh_1.0 phase-1 step-1-10 官方在线 runner:replay 模式(决策日志回放,LUT 时钟)。
...
DECISION_LOG=${3:?"decision_log 必填(request-neutral:请按方案文档 sh_1.0仓库改造详细执行方案.md §3 步骤 0-1 物化决策日志后显式传入)"}
...
if [[ ${CPP_EXIT} -ne 0 || ${PY_EXIT} -ne 0 ]]; then
  echo "[run_online_replay] FAIL: bridge retained for debugging (request=$(find "${RUN_DIR}/bridge" -maxdepth 1 -name 'request_*.json' 2>/dev/null | wc -l), jsonl=$(find "${RUN_DIR}/bridge" -maxdepth 1 -name '*.jsonl' 2>/dev/null | wc -l)); next run rm -rf clears it" >&2
fi
