...
# The generator dumps the full manifest JSON to stdout; keep it in a temp log
# so the terminal only shows the key status lines. stderr (progress and
# errors) still passes through.
TRACE_GEN_LOG=$(mktemp -t sh_test_generate_trace.XXXXXX.log)
trap 'rm -f "${TRACE_GEN_LOG}"' EXIT

if ! "${PYTHON_BIN}" "${TRACE_GENERATOR_ARG}" "${TRACE_GENERATOR_ARGS[@]}" >"${TRACE_GEN_LOG}"; then
  echo "[sh_test] Trace generation failed; generator output follows:" >&2
  cat "${TRACE_GEN_LOG}" >&2
  exit 1
fi

TRACE_CONFIG_ASSIGNMENTS=$("${PYTHON_BIN}" "${TRACE_GENERATOR_ARG}" --print-shell-config)
eval "${TRACE_CONFIG_ASSIGNMENTS}"
echo "[sh_test] Trace generated under: ${TRACE_DIR}"
