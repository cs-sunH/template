...
* **sh_2.0**：决策只落聚合 bytes；准入时点 history_location_before 未
  序列化（sh20_online_scheduler.py:1104-1167）→ kv_hit_state=
  not_supported（turn=0 除外）。TODO_S2。
* **sh_3.0**：prefill_affinity_reason 五值（sh30_online_scheduler.py:
  1150-1221）；resident_prefix_layers（PARTIAL_HBM_REMOTE）→ partial。
* **hopbytes 覆盖**：sh_1.0=shard 级 noc_path；wscllm=实例级
  static_route.hop_count（仅 PD 迁移）；face/sh_2.0/sh_3.0 产物无 hops
  字段 → coverage=0（TODO_FACE/TODO_S2/TODO_S3，不臆造）。
...
* **逐仓覆盖度**（B0 基线 60s_summary 实测）：
  ...
  * S2：`count_only`——decision log 只有 *_eviction_count（无 victim/
    bytes），他人逐出不可归因：occupancy 为上界（未归因逐出不扣减，B0 为
    238 例），violation 检查降级，occupancy_valid=false。自身会话去向经
    kv_location_after_completion 归因（remote=全量扣减；partial=比例未知，
    保留+计数，不臆造）。
