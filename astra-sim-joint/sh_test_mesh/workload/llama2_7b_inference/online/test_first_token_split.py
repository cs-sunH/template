#!/usr/bin/env python3
"""test_first_token_split.py -- WP9 首 token 首步批拆分钉子测试（S3 版，
2026-08-26；结构照母本 sh_1.0 同名测试移植，账本对象换为本仓 runtime）。
...
def _joiner_plan(request_id, context_tokens, decode_length=4):
    return {
        ...
        # local_hit 零迁移节点 ⇒ join 标记是唯一 decode_start 锚点；drain
        # 触发门给占位（发射不校验其值，仅入边）。
...
def _train_plan(train_id, spans, iterations, joiners=(), drains=(),
                ...
                first_chunk=None, instance_index=1, first_token=None,
                sentinel=False):
    plan = {
        "train_id": train_id,
...
class DebutPlanTest(unittest.TestCase):
    ...
    def test_no_debut_returns_none(self):
        ...
        self.assertIsNone(self.s._first_token_plan(self._plan(), [veteran]))
    ...
    def test_tmax_1_plus_7_split(self):
        """(c)：8 迭代（T_max=8 截断同构）拆 1+7——首步 = 首 chunk +
        各成员第 1 span；余量 = 7 chunk + 成员剩余 span；总量守恒。"""
        fresh = _runtime(REQUEST_B, consumed=0)
        self.s.runtime_by_request_id = {REQUEST_B: fresh}
    ...
    def test_decode_length_1_debut_uses_exit_marker(self):
        ...
        debut = _runtime(REQUEST_B, decode=1)
        self.s.runtime_by_request_id = {REQUEST_B: debut}
        plan = self._plan()
        token = self.s._first_token_plan(plan, [debut])
...
class SplitEmissionTest(unittest.TestCase):
    ...
    def test_split_batch_structure(self):
        ...
        self.assertEqual(
            ...
            sorted(DECODE_RANKS))
        # 首步批标记置位（digests first_step=True 的来源）。
        self.assertTrue(builder.batch_first_step)
        first_ops, first_bytes = _train_bytes(builder, "batch_train_i1_1")

        builder.begin_batch()
        self.assertFalse(builder.batch_first_step)
        ...
        self.assertEqual(
            builder._block_ends[REQUEST_A]["seg2"][DECODE_RANKS[0]],
    ...
    def test_split_plan_rejected_by_whole_emitter(self):
        """拆分计划交给整列发射器必须 fail-closed（防误用双发射）。"""
        split_first = _split_first_token(
            "batch_train_i1_1", [(1, 101), (1, 41)],
            [(1, 102), (1, 103), (1, 42)], 2,
            debut_markers=[{"request_id": REQUEST_B}])
        with self.assertRaises(RuntimeError):
            self._new_builder().emit_iteration_train(
                self._two_member_plan(split_first))
    ...
    def test_off_equivalence_byte_identical(self):
        ...
        try:
            ...
            self.assertIsNone(
                s._first_token_plan(
                    {"train_id": "t", "iterations": 4, "members": [],
                     "prefill_chunk_tokens": []}, [fresh]))
