...
§7.7 最小测试集在调度器机制层的合成覆盖,不跑仿真;照母本 sh_1.0 定型版
移植,类名/账本对象换为本仓 Sh30OnlineScheduler 的 _OnlineInstanceState
与 _OnlineRequestRuntime)。
...
def _runtime(request_id, *, ctx=100, decode=4, remaining_chunks=0,
             prefill_work=512, history=0, instance=0, p_chunk=512):
    """sh_3.0 的 _OnlineRequestRuntime 由 manifest record dict 构造
    （本仓账本对象形态）;构造后覆写 remaining_chunks 以支持合成场景。"""
    ...
    # 合成场景的显式覆写(母本 _OnlineRuntime 的显式 prefill_work/
    # remaining_chunks 入参同款):本仓构造器自 record 推导,此处对齐
    # 测试意图(prefill_work 与 ctx 独立给定)。
    rt.prefill_tokens_to_process = prefill_work
    rt.remaining_chunks = remaining_chunks
...
def _bare_scheduler(p_chunk=512):
    ...
    s.p_chunk = p_chunk
    s.instances = [_OnlineInstanceState(index=i) for i in range(2)]
...
class TrainPlanningTest(unittest.TestCase):
    ...
    def test_mixed_train_chunk_per_iteration_and_drain_boundary(self):
        ...
        s.runtime_by_request_id["rm"] = member
        ...
        self.assertEqual(plan["head_request_id"], "rh")
    ...
    def test_pure_prefill_train(self):
        ...
        s = _bare_scheduler()
        state = s.instances[0]
        rp = _runtime("rp", ctx=512, decode=1, remaining_chunks=2,
                      prefill_work=1024)
        s.runtime_by_request_id["rp"] = rp
        state.qp.append(rp)
        plan = s._plan_train(state)
    ...
    def test_stale_queue_head_is_skipped(self):
        """remaining_chunks==0 的头部跳过(drain 决策事件尚在途中):
        后续请求的 chunk 可开始,列车终于有效头部。"""
        s = _bare_scheduler()
        state = s.instances[0]
        stale = _runtime("rs", ctx=512, decode=1, remaining_chunks=0,
                         prefill_work=512)
        s.runtime_by_request_id["rs"] = stale
        state.qp.append(stale)
        head = _runtime("rh2", ctx=0, decode=2, remaining_chunks=1,
                        prefill_work=400, history=0)
        s.runtime_by_request_id["rh2"] = head
        state.qp.append(head)
        plan = s._plan_train(state)
        self.assertEqual(plan["iterations"], 1)
        self.assertEqual(plan["drain_members"], ["rh2"])
        self.assertEqual(plan["head_request_id"], "rh2")
        self.assertEqual(plan["pass_spans"], [(400, 400)])
...
class TrainFinalizeTest(unittest.TestCase):
    ...
    def test_same_tick_multi_instance_and_split_delivery_idempotent(self):
        ...
        s.runtime_by_request_id["md"] = rt_d
        ...
        self.assertEqual([rt.request_id for rt in st0.qp], ["md"])
        self.assertEqual(s.runtime_by_request_id["md"].remaining_chunks, 0)
    ...
    def test_busy_gate_skips_in_flight_instance(self):
        ...
        state = s.instances[0]
        head = _runtime("rz", ctx=0, decode=1, remaining_chunks=1,
                        prefill_work=300)
        s.runtime_by_request_id["rz"] = head
        state.qp.append(head)
        state.in_flight_train = {"train_id": "t", "iterations": 1,
                                 ...
                                 "signal_set": set(),
                                 "prefill_chunk_tokens": []}
        s._ready_frontier.add(0)  # 有工作且在 frontier;唯一跳过因 = busy 门
