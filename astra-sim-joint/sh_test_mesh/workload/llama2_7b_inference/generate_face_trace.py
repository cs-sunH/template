    ...
    for shard_index, shard in enumerate(transfer.shards):
        ...
        if transfer.kind == "noc_migrate":
            ...
            if source_rank == edge_rank:
                ...
                # KV-eviction parallelization (2026-09-13): return-record
                # extension only -- expose the branch tail node ids
                # (edge mem_store / source ack recv) for the builder-side
                # store->restore forward-dependency ledger. The emitted
                # nodes/edges above are byte-for-byte unchanged.
                ...
                # KV-eviction parallelization (2026-09-13): capture the
                # edge mem_store tail id immediately (previous_id advances
                # with the ack send below); return-record extension only,
