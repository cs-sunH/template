#!/usr/bin/env bash
# sh_3.0 phase-1 step-1-11(CORE)same-tick milestone fixture(方案 step 1-11
...
RUN_ROOT=${1:-/tmp/sh_3.0_same_tick_milestone}

# ET 目录动态解析(本仓官方 runner 同款;2026-08-16 缺陷修复2同步轮补齐——
# 本 fixture runner 曾硬编码旧 label 后缀目录名,配置字节变化即必断)。
GEN_MATCH=("${PROJECT}"/sh_test_mesh/generated/llama2_7b_inference_54npus_*)
if [[ ${#GEN_MATCH[@]} -ne 1 || ! -d "${GEN_MATCH[0]}" ]]; then
  echo "[fixture] expected exactly one generated dir under sh_test_mesh/generated, found: ${GEN_MATCH[*]}" >&2
  exit 1
fi
ET_DIR=${GEN_MATCH[0]}
