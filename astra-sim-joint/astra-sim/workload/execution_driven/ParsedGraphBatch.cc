...
ParsedNode parse_node(const nlohmann::json& node_json, size_t index,
    ...
    // N2: exactly the frozen 13-key set + this repo's optional "mem"
    // sub-object (sh_3.0's MemAttrs schema; the real graph_batch_builder
    // always emits it, pre-C1/legacy fixtures may omit it -- the old DOM
    // walk bound a kEmptyObject stand-in, so absence keeps the defaults).
    check_key_set(
        node_json,
    ...
    // sh_3.0's three emission boundaries: prefill / decode / completion
    // (the completion tail carries on from decode, so only prefill is
    // generation 0 -- identical to the pre-C1 committer rule).
    if (node.stage != "prefill" && node.stage != "decode" &&
        node.stage != "completion") {
    ...
    // {remote_weight_bytes} (sh_3.0: the HBM access mode lives in the "mem"
    // sub-object below, not under compute).
    ...
    // N12m (sh_3.0): mem -- the repo's MemAttrs sub-object; optional at the
    // node level and field-by-field (the pre-C1 walk read every key through
    // .value(key, default), so a missing key kept the default while a
    // present-but-mistyped one failed closed -- preserved here; only the
    // unknown-key rejection is the C1 tightening). The real
    // graph_batch_builder always emits all three keys.
    if (node_json.contains("mem")) {
        const nlohmann::json& mem = node_json["mem"];
        if (!mem.is_object()) {
            fail(where + ": mem is not an object");
        }
        check_key_set(mem,
                      {"tensor_size", "is_local_hbm_kv_restore",
                       "hbm_access_mode"},
                      {},
                      where + ".mem");
        if (mem.contains("tensor_size")) {
            node.mem.tensor_size =
                get_uint(mem["tensor_size"], "tensor_size", where + ".mem");
        }
        if (mem.contains("is_local_hbm_kv_restore")) {
            node.mem.is_local_hbm_kv_restore = get_bool(
                mem["is_local_hbm_kv_restore"], "is_local_hbm_kv_restore",
                where + ".mem");
        }
        if (mem.contains("hbm_access_mode")) {
            node.mem.hbm_access_mode = get_int(
                mem["hbm_access_mode"], "hbm_access_mode", where + ".mem");
        }
    }
