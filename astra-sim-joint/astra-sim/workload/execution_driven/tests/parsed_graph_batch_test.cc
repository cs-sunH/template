...
nlohmann::json compute_full() {
    return {{"num_ops", 5000000000ULL},      // > 2^32: uint64 must survive
            ...
            {"remote_weight_bytes", 777}};
}

// sh_3.0's MemAttrs sub-object (optional at the node level, all keys
// optional within it -- the pre-C1 .value(key, default) semantics).
nlohmann::json mem_full() {
    return {{"tensor_size", 4096},
            {"is_local_hbm_kv_restore", true},
            {"hbm_access_mode", 2}};
}
...
void test_node_rules() {
    ...
    {  // N2: unknown node key
        nlohmann::json resp = base;
        resp["nodes"][0]["memo"] = nlohmann::json::object();
    ...
    {  // N12: compute key set / domains
        ...
        resp = base;
        // sh_3.0: hbm_access_mode is NOT a compute key (it belongs to the
        // mem sub-object) -- the parse must reject it as unknown.
        resp["nodes"][0]["compute"]["hbm_access_mode"] = 1;
        expect_parse_error(resp, "N12: compute unknown hbm_access_mode",
                           "hbm_access_mode");
    }
    {  // N12m: sh_3.0 mem sub-object -- optional node key, optional fields,
        //  unknown keys / wrong types / non-object rejected; a fully absent
        //  mem keeps the defaults (pre-C1 kEmptyObject semantics).
        nlohmann::json resp = base;
        resp["nodes"][0].erase("mem");
        const ParsedGraphBatch no_mem = parse_ok(resp, "N12m: absent mem");
        expect(no_mem.nodes[0].node.mem.tensor_size == 0 &&
                   !no_mem.nodes[0].node.mem.is_local_hbm_kv_restore &&
                   no_mem.nodes[0].node.mem.hbm_access_mode == 0,
               "N12m: absent mem keeps the MemAttrs defaults");
        resp = base;
        resp["nodes"][0]["mem"] = "junk";
        expect_parse_error(resp, "N12m: mem not an object", "mem");
        resp = base;
        resp["nodes"][0]["mem"]["dma"] = 1;
        expect_parse_error(resp, "N12m: mem unknown key", "dma");
        resp = base;
        resp["nodes"][0]["mem"]["tensor_size"] = -1;
        expect_parse_error(resp, "N12m: mem negative tensor_size",
                           "negative");
        resp = base;
        resp["nodes"][0]["mem"]["hbm_access_mode"] = "read";
        expect_parse_error(resp, "N12m: mem hbm_access_mode string",
                           "hbm_access_mode");
        resp = base;
        resp["nodes"][0]["mem"].erase("tensor_size");
        const ParsedGraphBatch partial_mem =
            parse_ok(resp, "N12m: mem partial fields tolerated");
        expect(partial_mem.nodes[0].node.mem.tensor_size == 0 &&
                   partial_mem.nodes[0].node.mem.is_local_hbm_kv_restore,
               "N12m: partial mem keeps per-field defaults");
    }
...
void test_max_shape_round_trip() {
    ...
    const MemAttrs& mem = parsed.nodes[0].node.mem;
    expect(mem.tensor_size == 4096 && mem.is_local_hbm_kv_restore &&
               mem.hbm_access_mode == 2,
           "P: sh_3.0 mem sub-object round-trips");
