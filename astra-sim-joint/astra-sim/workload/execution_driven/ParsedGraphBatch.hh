...
other bridge protocol violation):
  ...
  N1..N15 node rules: exact 13-key set + this repo's optional "mem"
     sub-object, rank/id/type/name/flag/string/request/stage/generation
     typing and domains (sh_3.0 stages: prefill/decode/completion, only
     prefill is generation 0), compute/mem/comm/coll sub-object key sets
     (required + allowed), integer-overflow rejection
     (a JSON integer literal > UINT64_MAX parses as a double and fails
     is_number_integer), negative-value rejection, tag/priority <=
     UINT32_MAX. The mem sub-object (sh_3.0 MemAttrs: local-HBM / KV-restore
     attributes) carries no port-liveness rule in this repo -- the
     preflight keeps every state-dependent rule it owned.
