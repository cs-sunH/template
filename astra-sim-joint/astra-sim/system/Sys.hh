...
class Sys : public Callable {
  public:
    ...
    // Class-internal safe initialization: the JSON input must provide
    // "scheduling-policy" (initialize_sys fails fast when the key is
    // missing); this default only exists so an unconfigured read can never
    // observe an indeterminate enum value.
    SchedulingPolicy scheduling_policy = SchedulingPolicy::None;
    ...
    // Class-internal safe initialization against an uninitialized read when
    // the JSON input omits "collective-optimization" (in-repo minimal test
    // configurations legitimately do so); Baseline keeps the conservative
    // two-phase-all-dimensions behavior.
