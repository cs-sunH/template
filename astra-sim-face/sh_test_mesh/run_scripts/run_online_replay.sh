#!/usr/bin/env bash
# face phase-1 step-1-10 官方在线 runner:replay 模式(决策日志回放,LUT 时钟)。
# Usage: bash run_online_replay.sh <run_dir> <request_csv> <decision_log>
# request-neutral(裸仓库):request_csv 与 decision_log 由调用方按方案文档
# §3 步骤 0-1 物化后必填传入(缺失即 fail-closed,不落默认)。
# 流程:启 Python 服务(后台,等 C++ 建好桥接 FIFO)-> 跑在线二进制 ->
#   等退出码 -> 收日志 -> 后处理([METRIC] 行经 run_metrics_postprocess.sh
#   产出 raw_metrics.csv / normalized_metrics.csv)。
set -euo pipefail
...
RUN_DIR=$1
# 阶段 7 §10.4:窗口扫掠旋钮(WINDOW_ROWS 环境变量 -> --request-window-rows;
# 缺省不传 = C++ 默认 128)。扫掠口径:不同窗口下 online_decision_log 逐字节
# 一致(零扰动)。
WINDOW_ROWS_ARGS=""
if [[ -n "${WINDOW_ROWS:-}" ]]; then
  WINDOW_ROWS_ARGS="--request-window-rows=${WINDOW_ROWS}"
fi
REQUEST_CSV=${2:?"request_csv 必填(request-neutral:请按方案文档 face仓库改造详细执行方案.md §3 步骤 0-1 物化输入后显式传入)"}
DECISION_LOG=${3:?"decision_log 必填(request-neutral:请按方案文档 face仓库改造详细执行方案.md §3 步骤 0-1 物化决策日志后显式传入)"}
...
until [ -p "${RUN_DIR}/bridge/req_notify.fifo" ] && [ -p "${RUN_DIR}/bridge/resp_notify.fifo" ]; do
  if ! kill -0 "${CPP_PID}" 2>/dev/null; then
    echo "[run_online_replay] C++ exited before FIFO ready (see cpp.log)" >&2
    exit 1
  fi
  sleep 0.3
done
...
echo "[run_online_replay] cpp_exit=${CPP_EXIT} python_exit=${PY_EXIT}"
...
if [[ ${CPP_EXIT} -ne 0 || ${PY_EXIT} -ne 0 ]]; then
  echo "[run_online_replay] FAIL: bridge retained for debugging (request=$(find "${RUN_DIR}/bridge" -maxdepth 1 -name 'request_*.json' 2>/dev/null | wc -l), jsonl=$(ls "${RUN_DIR}/bridge/"*.jsonl 2>/dev/null | wc -l)); next run rm -rf clears it" >&2
fi
...
if grep -q '\[METRIC\]' "${RUN_DIR}/cpp.log"; then
  ...
  echo "[run_online_replay] metrics postprocess: raw=$(ls "${RUN_DIR}/raw_metrics.csv") normalized=$(ls "${RUN_DIR}/normalized_metrics.csv")"
else
  echo "[run_online_replay] WARNING: no [METRIC] lines in cpp.log; postprocess skipped" >&2
fi
...
echo "[run_online_replay] artifacts: ${ARCHIVED} jsonl archived -> results/; checkpoints=${CP_COUNT}; request retained=${REQ_COUNT}"
echo "[run_online_replay] PASS: ${RUN_DIR}"
