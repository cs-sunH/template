...
echo "[sh_test] Scenario: ${MODEL_NAME} (${MLP_VARIANT}), ${HARDWARE_LABEL}, ${NPUS_COUNT} NPUs on a ${MESH_SHAPE} mesh, ${INSTANCE_COUNT} unified TP=${TP_DEGREE} instances; p_chunk=${PREFILL_CHUNK_SIZE}; trace granularity=${TRACE_GRANULARITY}; Prefill uses remaining-chunk/last-arrival ordering, Decode uses weighted Instance_map plus minimum per-die LUT delta, and KV remains on wafer with local-first allocation; ${REQUEST_COUNT} requests across ${SESSION_COUNT} sessions, prefill ${PREFILL_RANGE}, decode ${DECODE_RANGE}."
"${ASTRA_SIM}" \
