...
def build_trace_label(config: FaceTraceConfig, plan: FacePlan) -> str:
...
def resolve_output_dir(config: FaceTraceConfig, plan: FacePlan) -> Path:
    ...
    *,
    ...
    return {
        ...
        "prefill_assignment": {
            ...
            "policy": "remaining_chunks,last_arrival_time,config_order",
        },
        "decode_assignment": {
            ...
            "policy": "min((T_prime-T)/instance_size),config_order",
        },
            ...
            writer.writerow(
                {
                    ...
                    "source_instance_index": (
                        "" if event.source_instance_index is None else event.source_instance_index
                    ),
                    "target_instance_index": (
                        "" if event.target_instance_index is None else event.target_instance_index
                    ),
                    "context_tokens": event.context_tokens,
                    "total_bytes": event.total_bytes,
                    "shard_bytes": json.dumps(list(event.shard_bytes)),
                    "last_completion_ns": (
                        "" if event.last_completion_ns is None else event.last_completion_ns
                    ),
                    "instance_remaining_before_bytes": json.dumps(
                        list(event.instance_remaining_before_bytes)
                    ),
                    "instance_remaining_after_bytes": json.dumps(
...
def _session_lru_request_record(
    request: RequestSpec,
    request_plan: FaceRequestPlan,
    plan: FacePlan,
    history_routes: list[dict[str, object]],
    ...
    return {
        ...
        "history_transfer": _transfer_dict(request_plan.history_transfer),
        "prefill_decode_transfer": _transfer_dict(request_plan.prefill_decode_transfer),
        "history_routes": history_routes,
        "prefill_decode_routes": prefill_decode_routes,
        "evicted_sessions_before_prefill": [
            _eviction_dict(record) for record in request_plan.admission_evictions
        ],
        "evicted_sessions_before_decode": [
            _eviction_dict(record) for record in request_plan.decode_target_evictions
        ],
        "evicted_sessions_after_completion": [
            _eviction_dict(record) for record in request_plan.completion_evictions
        ],
        ...
        "hbm_before_request": [
            _hbm_snapshot_dict(snapshot) for snapshot in request_plan.hbm_before_request
        ],
        "hbm_after_completion": [
            _hbm_snapshot_dict(snapshot) for snapshot in request_plan.hbm_after_completion
        ],
        "planned_timing_ns": {
            "arrival": request_plan.estimated_arrival_ns,
            "prefill_start": request_plan.prefill_start_ns,
...
def _write_face_session_lru_trace(
    config: FaceTraceConfig,
    plan: FacePlan,
    *,
    ...
    try:
        ...
        ordered_plans = sorted(plan.requests, key=lambda item: (item.prefill_start_ns, item.queue_index))
        for request_plan in ordered_plans:
            ...
            if request_plan.turn_index == 0:
                ...
                timers = {
                    rank: builders[rank].timer_gate(
                        f"{prefix}_arrival_timer_gate", request.session_arrival_time_ns
                    )
                ...
                if previous is None or request.inter_request_interval_ns is None:
                    raise RuntimeError("later request has no previous completion gate")
                previous_instance_index, previous_nodes = previous
                previous_group = group_by_index[previous_instance_index]
                timers = {
                    rank: builders[rank].timer_gate(
                        f"{prefix}_interval_timer_gate",
                ...
                if request_plan.history_action == NOC_MIGRATE:
                    ...
                    _emit_control_trigger(
                        builders=builders,
                        queue_index=request_plan.queue_index,
                        name=f"{prefix}_interval_control",
                        source_group=previous_group,
                        target_group=prefill_group,
                        timer_gates=timers,
                    )
            ...
            if request_plan.history_action == RECOMPUTE:
                recompute_bounds = _emit_prefill_stage(
                    ...
                    group=prefill_group,
                    prefix=prefix,
                    stage="history_recompute",
                    tokens=request_plan.history_recompute_tokens,
                    initial_context_tokens=0,
                )
            for rank in prefill_group.ranks:
                builders[rank].all_reduce(
                    f"{prefix}_history_tp_ready_barrier", 1, prefill_group.pg_name
                )
            ...
            if config.trace_granularity == "token_expanded":
                for relative_rank, rank in enumerate(decode_group.ranks):
                    ...
                    builders[rank].all_reduce(
                        f"{prefix}_decode_request_end_barrier", 1, decode_group.pg_name
                    )
                    if metrics is not None:
                        metrics.add_event(
                            rank,
            ...
            request_records.append(
                _session_lru_request_record(
                    request,
                    request_plan,
                    plan,
                    history_routes,
                    prefill_decode_routes,
                )
            )
        ...
        for output in outputs.values():
            output.close()
        outputs.clear()
        plan.lut.export_csv(staging / "face_lut.csv")
        ...
        local_hit_count = sum(event.event_type == "local_hit" for event in plan.kv_events)
        noc_migrate_count = sum(event.event_type == "noc_migrate" for event in plan.kv_events)
        recompute_count = sum(event.event_type == "recompute" for event in plan.kv_events)
        eviction_count = sum(event.event_type == "evict_delete" for event in plan.kv_events)
        blocked_count = sum(event.event_type == "admission_blocked" for event in plan.kv_events)
        final_resident = sum(snapshot.state == "RESIDENT" for snapshot in plan.final_session_snapshots)
        if metrics is not None:
        ...
        manifest = {
            ...
            "execution_mode": "face_session_lru_recompute_static_et",
            ...
            "kv_management": {
                ...
                "noc_migrate_count": noc_migrate_count,
                "recompute_count": recompute_count,
                "eviction_count": eviction_count,
                "admission_blocked_count": blocked_count,
                "final_resident_session_count": final_resident,
                "final_evicted_session_count": len(plan.final_session_snapshots) - final_resident,
                "final_hbm_snapshots": [
                    _hbm_snapshot_dict(snapshot) for snapshot in plan.final_hbm_snapshots
                ],
                "final_session_snapshots": [
                    {
                        "session_id": snapshot.session_id,
                        "logical_context_tokens": snapshot.logical_context_tokens,
                        ...
                        "evicted_at_ns": snapshot.evicted_at_ns,
                        "evicted_by_request_id": snapshot.evicted_by_request_id,
                    }
                    for snapshot in plan.final_session_snapshots
                ],
            },
            "hardware": {
...
def write_face_trace(
    ...
    *,
    ...
    if config.kv_cache_policy == "session_lru_recompute":
        _write_face_session_lru_trace(config, plan, metrics=metrics)
        return
    if metrics is not None:
    ...
    manifest = {
        ...
        "mapping_strategy": (
            ...
            "D2D/DRAM; LUT minimum per-die incremental decode latency; local-first KV"
        ),
        ...
        "kv_management": {
            ...
            "offload_edge_weight_increment": 1,
            "session_history": "cumulative_KV_retained_on_wafer_until_next_turn",
...
def print_shell_config(config: FaceTraceConfig, plan: FacePlan) -> None:
    output_dir = resolve_output_dir(config, plan)
    session_count = len({request.session_id for request in config.request_queue})
    assignments = {
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    if metrics is not None:
        ...
        face_scheduler.set_iteration_stats_hook(lut_stats.record_lut_iteration)
    try:
        plan = build_face_plan(config)
    finally:
        session_kv_manager.set_metrics_observer(None)
        face_scheduler.set_iteration_stats_hook(None)
    if print_shell:
