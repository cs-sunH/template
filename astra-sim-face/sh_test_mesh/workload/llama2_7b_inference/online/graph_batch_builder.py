...
(_write_face_session_lru_trace, generate_face_trace.py:1075-1376)由模块级
...
class GraphBatchBuilder:
    ...
    def _emit_prelim(self, request_plan: dict) -> dict:
        ...
        # face 独有:统一实例的 decode 实例在 PREFILL_DRAIN 边界才选定
        # (select_decode_instance),prefill 发射时 decode_instance_index 可能为
        # None——此时块末账本只登记 prefill 组 rank;decode 组 rank 的块末 =
        # None(decode 侧首节点 recv 无父,与离线首个 decode 的 recv 同构,
        # 蓝本裁决 9 的 None-restore 语义)。
        if request_plan.get("decode_instance_index") is None:
            covered_ranks = set(prefill_group.ranks)
        else:
            decode_group = self.group_by_index[
                request_plan["decode_instance_index"]]
            covered_ranks = set(prefill_group.ranks) | set(decode_group.ranks)
        self._prefill_block_ends[request_plan["request_id"]] = {
            rank: (builders[rank].previous_id if rank in prefill_group.ranks
                   else None)
            for rank in sorted(covered_ranks)
        }
        return members
    ...
    def _emit_decode(self, request_plan: dict) -> dict:
        ...
        # face:decode 组 rank 未登记(prefill 发射时 decode 未选定)时按
        # None 恢复(recv 无父,同上);显式归一 decode 组全部 rank,防 stale
        # 跨 request previous_id 残留。
        for rank in decode_group.ranks:
            if rank not in (block_ends or {}):
                builders[rank].previous_id = None
        _paired_transfer(
            config=self.config, builders=builders,
    ...
    def emit_prefill_batch_legacy(self, request_plan: dict) -> dict:
        ...
        # decode 块末账本(裁决 9 None-restore;legacy decode 组 rank 在
        # prefill 发射时未知)。
        if request_plan.get("decode_instance_index") is None:
            covered = set(prefill_group.ranks)
        else:
            covered = set(prefill_group.ranks) | set(
                self.group_by_index[
                    request_plan["decode_instance_index"]].ranks)
        self._prefill_block_ends[request_plan["request_id"]] = {
            rank: (builders[rank].previous_id
                   if rank in prefill_group.ranks else None)
            for rank in sorted(covered)
        }
        self._collect(marker)
        return members
