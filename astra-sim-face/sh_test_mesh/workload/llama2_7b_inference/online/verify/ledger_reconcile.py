...
def load_responses(bridge_dir, manifest_path=None):
    ...
    if not _seq_files(bridge_dir, "response_"):
        ...
        for _seq in _seq_files(bridge_dir, "request_"):
            with open(os.path.join(bridge_dir,
                                   "request_{}.json".format(_seq)),
                      "r", encoding="utf-8") as _src:
                _delta = json.load(_src)
...
def load_digests(run_bridge_dir):
    ...
    fail-closed(不允许静默跳过)。"""
    path = os.path.join(run_bridge_dir, "graph_batch_digests.jsonl")
