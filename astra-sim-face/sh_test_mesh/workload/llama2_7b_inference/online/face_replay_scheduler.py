#!/usr/bin/env python3
"""face_replay_scheduler.py -- 决策日志回放调度器(replay 模式)。
...
流序匹配语义 = 蓝本裁决 12 终版(到达条件集 + 头部推进):到达事件只登记;
游标推进时头部记录所属请求必须在 arrived 集合内,否则才失步。face 与
wscllm 同款装置语义(共享机制层,face 版逐仓实现)。
...
# face 30s 输入的 q 分布以实测为准(蓝本 30s 实测 q<=17ns 恒不触发,
# 3min q 达 23.6s 全部吸收;face 装置同款阈值)。
_DEFER_THRESHOLD_NS = 1000


class FaceReplayScheduler(OnlineSchedulerBase):
    ...
    def run_variant_policy(self, delta) -> None:
        ...
        for group in delta["completed_groups"]:
            ...
            if stage == STAGE_PREFILL:
                ...
                # 下一次 arrival 排程由同 tick 一并交付的 REQUEST_COMPLETE
                # 处理)。
