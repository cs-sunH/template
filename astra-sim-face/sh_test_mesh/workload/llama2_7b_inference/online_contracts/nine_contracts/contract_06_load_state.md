...
分层账本八项(总体方案 §5.2/§5.6;face 裁剪):

| 层 | 定义(face) | 维护方 | face 状态 |
|---|---|---|---|
...
| issued | 已发射执行 | C++ | 启用 |
| remote FIFO | 远端内存排队 | — | **不适用(显式占位;face 无远端内存)** |
| network pending/active | 网络层在途 | C++ FluidScheduler | 启用(计数;策略不消费,见阶段 3) |
| local HBM job | 本地 HBM 执行队列 | — | **不适用(显式占位;face 无 LocalHbmBandwidthModel,全仓 grep 零命中)** |
...
- face 策略输入全是 Python 账本(排队深度 + KV 容量 + 静态路由),
