...
token):
  ...
  R12 M2 node GC (2026-08-23): --online-node-gc <0|1> with frozen default
       0 (off -- ruling flip 2026-08-23: GC on showed a reproducible
       light-load wall regression); 1 = collection arm for memory-bound
       heavy/parallel runs; 0 = pre-M2 never-erase rollback arm; both
...
int main() {
    ...
    // 0 (off -- flipped 2026-08-23 after the reproducible light-load wall
    // regression; face port fixes this assertion to the flipped default);
    // 1 = collection arm for memory-bound heavy/parallel runs; 0 = pre-M2
    // never-erase behavior; both value forms legal; garbage / missing
