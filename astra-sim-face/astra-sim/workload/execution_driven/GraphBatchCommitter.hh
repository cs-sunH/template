...
class GraphBatchCommitter {
  public:
    ...
    struct Context {
        ...
        // one of those Workloads exposes the same declaration for pg_name,
        // and returns that declaration in its preserved rank order.  Returning
        // nullopt means missing/inconsistent metadata and fails a collective
        // batch closed before any commit state changes.  Batches without
        // collective nodes do not require the callback.
    ...
    /// collective membership, and future-alarm accounting) before it changes
    /// any state.
