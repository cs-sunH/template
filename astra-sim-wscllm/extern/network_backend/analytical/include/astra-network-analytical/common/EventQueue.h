...
class EventQueue {
  public:
    ...
    /**
     * Set the tick-end callback (phase-1 execution-driven extension).
     *
     * When set, the callback is invoked exactly once per proceed(), after the
     * current tick's physical EventList has been popped and before the same-tick
     * deferred queue drains. When not set (nullptr, the default) the queue
     * behaves byte-for-byte like the pre-extension implementation.
     *
     ...
     * schedule_event_deferred(); schedule_event(current_time, ...) would insert
     * a current_time EventList into the main queue and trip the strict-increase
     * assert on the next proceed().
     *
     ...
     * drain pass. Deferred events belong to the current tick: they must not be
     * used for future times (use schedule_event for those).
     *
     * @param callback callback function pointer
     * @param callback_arg argument of the callback function
     */
     ...
     * (phase-1 execution-driven extension; defect-C fix 2026-08-16).
     *
     * The deferred queue only drains inside proceed(); a deferred event
     * scheduled outside a proceed context (or left over after one) is
     * INVISIBLE to finished() -- the main list can be empty while deferred
