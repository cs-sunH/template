...
TEST_F(TestNetworkAnalyticalCongestionUnaware, Mesh2D) {
    ...
    // 0 -> 15 in 4x4 row-major mesh uses 3 hops in dim 0 and 3 hops in dim 1.
    // The congestion-unaware model keeps one serialization delay per dimension.
    const auto comm_delay = topology->send(0, 15, chunk_size);
    EXPECT_EQ(comm_delay, 42'062);
}

TEST_F(TestNetworkAnalyticalCongestionUnaware, Mesh2DOneDim) {
    // create network
    const auto network_parser = NetworkParser("../../input/Mesh2D.yml");
    const auto topology = construct_topology(network_parser);

    // 0 -> 3 is a 3-hop line transfer in dim 0.
