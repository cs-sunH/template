...
# Phase-0 performance counter framework (plan step 0-7; 总改造计划 §6 阶段 0):
# named per-category counters for the stage-6/7 accounting.  The offline
# static-ET path never exercises callback/poll/bridge/GIL/log/reader-window/
# backpressure mechanisms and batching is not a Python decision boundary, so
# every counter stays 0 here.  The structure is observation-side only and is
# never serialized into any output (byte-equivalence gate: instrumentation
# must not change a single production byte).
@dataclass(frozen=True)
class Phase0Counters:
    callback_count: int = 0        # Python decision callbacks
    poll_count: int = 0            # completion polling hits
    bridge_bytes: int = 0          # Python<->C++ bridge payload bytes
    gil_contention_ns: int = 0     # GIL wait time in the bridge
    log_bytes: int = 0             # decision/audit log bytes written
    node_completion_count: int = 0  # node completion events observed
...
def _write_replay_decision_log(plan: WscLlmPlan, path: Path) -> None:
    ...
    ``prefill``/``decode``/``completion`` (per-request decisions; decision =
    prefill instance, decode route, admission, KV action sequence) or
    ``iteration`` (per-chunk WscLlmIterationRecord, i.e. the offline event
    loop timeline).  ``decision`` content follows the plan: prefill 实例、
    decode 路由、准入与否、KV 动作序列.
    """
        ...
        records.append(
            {
                ...
                "decision": {
                    ...
                    "admission_evictions": [
                        _eviction_record_dict(eviction)
        ...
        records.append(
            {
                ...
                "decision": {
                    ...
                    "decode_queue_depth_before_enqueue": (
                        request.decode_queue_depth_before_enqueue
                    ),
        ...
        records.append(
            {
                ...
                "decision": {
                    "terminal_kv_release_at_completion": (
                        request.terminal_kv_release_at_completion
                    ),
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    # The recording is a pure observer: iteration_count advances and every
    # planning decision is identical with or without it, so manifest.json and
    # all .et files remain byte-identical to the archived baseline.
    if replay_record:
