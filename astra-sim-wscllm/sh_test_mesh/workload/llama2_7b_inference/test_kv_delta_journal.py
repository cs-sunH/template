...
class KvDeltaJournalTests(unittest.TestCase):
    ...
    def test_conservation_matrix_full_lifecycle(self) -> None:
        ...
        # 每行前后快照自洽:after = before + delta(逐 rank 重放推导)。
        running: dict[int, list[int]] = {}
        for row in rows:
            state = running.setdefault(row["rank"], [0, 0, 0])
            self.assertEqual(
                [row["before_bytes"]["weight"],
                 row["before_bytes"]["resident"],
                 row["before_bytes"]["reserved"]], state)
            ...
            self.assertEqual(
                [row["after_bytes"]["weight"],
                 ...
                 row["after_bytes"]["reserved"]], state)
        # 终态守恒:只剩权重。
    ...
    def test_evict_recompute_and_terminal_retire_transactions(self) -> None:
        # 容量收到每 rank weight+20:一个 4-token 会话(16B)驻留后,第二个
        # 会话增长必然逐出前者(LRU completed inactive),随后被逐会话的
        # 下一 turn 走 RECOMPUTE(再次逐出现驻留者)。
        ...
        manager.grow_prefill("sB", 4, 14, "rB0")  # 逐出 sA
        manager.mark_complete("sB", 15, "rB0")
        manager.prepare_history("sA", 0, 4, 16, "rA1")  # RECOMPUTE,逐出 sB
        manager.grow_prefill("sA", 4, 17, "rA1")  # 零 delta 增长(零行事务)
        manager.mark_complete("sA", 18, "rA1")
        manager.retire_terminal_session("sA", 18, "rA1")
        manager.retire_terminal_session("sB", 15, "rB0")
        manager.assert_final_state()

        rows = _journal_rows(recorder)
        causes = [row["cause"] for row in rows]
        self.assertTrue(any("evict_delete" in cause for cause in causes))
        self.assertTrue(
            any("recompute_history_restore" in cause for cause in causes))
        self.assertTrue(
