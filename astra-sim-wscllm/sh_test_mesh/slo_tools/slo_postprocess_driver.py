...
6 / G5 legacy 变体跳步 9。shell 已探测时经 SH_SLO_HAVE_RM /
    ...
    if have_tl is None:
        ...
        if not have_tl:
            warn("results/train_ledger.jsonl absent — load_imbalance.py "
                 "skipped (by design; legacy variant produces no train "
                 "ledger)")
    skip_hbm = _env_gate("SH_SLO_SKIP_HBM")
    if skip_hbm is None:
        # B4b/2026-09-02：relevant_distributed 变体同样跳过——其决策日志含
        # run_header/kv_placement/kv_scatter/kv_remote_reads 新行（无
        # session-KV 逐出语义），session-KV 水位重放的对照列不适用；容量
        # 合规由调度器 run-end journal 守恒门（verify_relevant_journal_
        # conservation，逐 rank resident/reserved/physical=weight）承载。
        skip_hbm = 1 if any(
            (run_dir / "results" / name).is_file()
            for name in ("kv_event_payload_legacy.json",
                         "kv_event_payload_relevant.json")) else 0
        if skip_hbm:
            warn("variant with non-session-KV memory semantics detected "
                 "(results/kv_event_payload_{legacy,relevant}.json) — "
                 "hbm_watermark.py skipped (allocator semantics not covered "
                 "by session-KV replay)")
