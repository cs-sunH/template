...
# 设计，不算失败）；metrics=off 无任何 [METRIC] 行时整步跳过。legacy 变体
# （run_dir 含 results/kv_event_payload_legacy.json）额外跳过 load_imbalance
# （无列车台账）与 hbm_watermark（legacy 分配器语义不被 session-KV 重放
# 覆盖），同样写说明、不算失败。
set -uo pipefail
...
# 输入探测（legacy 变体，任务3 2026-08-28）：legacy allocator run 的 run_dir
# 标志 = results/kv_event_payload_legacy.json（legacy runner 归档件）。
# - load_imbalance 需要 results/train_ledger.jsonl（列车台账）——legacy 变体
#   不产列车（legacy runner 归档清单本就无 train_ledger），缺失 → 跳过；
# - hbm_watermark 输入在 legacy run 全在场，但 legacy 分配器（KVAllocator
#   跨实例分片）语义不被 session-KV 重放口径覆盖（会以负占用 fail-closed
#   退出，属预期不可重建）→ 探测到 legacy 即跳过。均写说明、不算失败。
TRAIN_LEDGER=${RUN_DIR}/results/train_ledger.jsonl
if [ -f "${TRAIN_LEDGER}" ]; then
  HAVE_TL=1
else
  HAVE_TL=0
  warn "results/train_ledger.jsonl absent — load_imbalance.py skipped (by design; legacy variant produces no train ledger)"
fi
if [ -f "${RUN_DIR}/results/kv_event_payload_legacy.json" ]; then
  SKIP_HBM=1
  warn "legacy variant detected (results/kv_event_payload_legacy.json) — hbm_watermark.py skipped (legacy allocator semantics not covered by session-KV replay)"
elif [ -f "${RUN_DIR}/results/kv_event_payload_relevant.json" ]; then
  # B4b/2026-09-02：relevant_distributed 变体同款跳过——决策日志含
  # run_header/kv_placement/kv_scatter/kv_remote_reads 新行（无 session-KV
  # 逐出语义），session-KV 水位重放口径不适用；容量合规由调度器 run-end
  # journal 守恒门（verify_relevant_journal_conservation）承载。
  SKIP_HBM=1
  warn "relevant_distributed variant detected (results/kv_event_payload_relevant.json) — hbm_watermark.py skipped (per-rank ledger semantics not covered by session-KV replay)"
else
  SKIP_HBM=0
fi
