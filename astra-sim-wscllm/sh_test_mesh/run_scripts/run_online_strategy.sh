...
# 楔死形态）——后者由 --idle-watchdog-s 墙钟停泊看门狗兜住（wscllm 无
# input-open 死端分支，合同 §2.1；缺省关），不在本变量职责内。
...
# --request-window-rows 缺省 128）。wscllm（sync-A16 批次4）：本仓窗口为
...
# P1(2026-08-30)kv delta journal 的 run 末 checksum 门产物(journal 开关
# off 或 legacy 变体时不存在,缺文件不是错误)。
