...
ASTRA_INTERNAL_LOG_DIR="${RESULT_DIR}/astra_internal_log_aware_wsc_llm"
...
if [[ ! -f "${REMOTE_MEMORY}" ]]; then
  echo "Missing NO_MEMORY_EXPANSION configuration: ${REMOTE_MEMORY}" >&2
  exit 1
fi
...
echo "[sh_test] Scenario: ${MODEL_NAME} (${MLP_VARIANT}), ${HARDWARE_LABEL}, WSC-LLM PD separation on the ${NPUS_COUNT}-NPU ${MESH_SHAPE} wafer mesh; ${PREFILL_INSTANCE_COUNT} Prefill-only plus ${DECODE_INSTANCE_COUNT} Decode-only TP=${TP_DEGREE} instances (${STATIC_PD_ROUTES}); p_chunk=${PREFILL_CHUNK_SIZE}; trace granularity=${TRACE_GRANULARITY}; Prefill uses least queued-request count with config-order ties, Decode uses the offline static route without runtime reselection, and KV uses the fixed same-Decode Relevant(P,D) domain; ${REQUEST_COUNT} requests across ${SESSION_COUNT} sessions, prefill ${PREFILL_RANGE}, decode ${DECODE_RANGE}."
"${ASTRA_SIM}" \
