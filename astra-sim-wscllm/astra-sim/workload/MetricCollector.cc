...
void MetricCollector::initialize(const std::string& manifest_path,
    ...
    if (this->kv_policy_.has_value()) {
        init_record["kv_policy"] = this->kv_policy_.value();
    }
    fflush(stdout);
...
void MetricCollector::load_manifest(const std::string& manifest_path) {
    ...
    if (manifest.contains("kv_policy") && manifest["kv_policy"].is_string()) {
        // Doc sec.9.2 (WSC-LLM): machine-readable KV policy from the
        // generator manifest, not from the scenario banner.
        this->kv_policy_ = manifest["kv_policy"].get<std::string>();
    }
