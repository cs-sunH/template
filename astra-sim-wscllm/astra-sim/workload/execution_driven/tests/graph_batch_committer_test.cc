...
future arrival) through:
  ...
  Part I  拼 batch §3.1 前置验证,2026-08-22 (sh_1.0 母本 Part H 的本仓
          对应; wscllm 的 Part H 字母位已被 hbm_charge 钉子占用): a
          two-request decode train (shared aggregate body nodes under the
          batch-namespace request_id "batch_train_0_1" -- NOT a real
          request -- plus one exit marker per member per rank and one
          per-train ALL_REDUCE end barrier) walks validate() + commit() +
          the full watch-fire chain under the one-directional coverage
          rule (every watch needs node coverage; namespace body / barrier
          nodes carry NO watch): both member decode watches fire, the
          namespace body / barrier terminals feed nothing, in-flight /
          prefill-drained tracking and the counters stay exact. Also proves
          the one rule that DOES guard the schema boundary: a member
          decode watch whose prefill never drained (no PREFILL_DRAIN delta
          fact) is still rejected with zero side effects.
  Part J  拼 batch §3.1 前置验证,2026-08-22 (sh_1.0 母本 Part I 对应;
          committer 机制层能力件 -- wscllm 策略层按 §3.6 PD 分离只发纯
          decode 列车, 混合 drain+exit 形态不会由本仓 Python 产出): one
          train carrying BOTH a drain marker (prefill watch, gen 0) and an
          exit marker (decode watch, gen 1) of two DIFFERENT requests --
          the mixed prefill-chunk + decode-token folding case; both watches
          fire.
...
// --------------------------------------------- 拼 batch 列车夹具 (Part I/J)
// The iteration-train node shapes the online builder emits (graph_batch_
// builder.emit_iteration_train): a batch-namespace shared body, one small
// marker per member per rank, and one per-train ALL_REDUCE end barrier
// (bytes = iteration count); stage "decode" is gen 1, "prefill" gen 0.
...
// ----------------------------------------------------------- Part I ------
// The two-request decode train (sh_1.0 母本 Part H 对应; snapshot-relative
// assertions -- the wscllm part lineage differs from the mother's). Epoch
// i1 lands the arrivals + req_A's prefill drain (a zero-node accounting
// batch), epoch i2 commits the train (req_B's prefill drain is a delta
...
// decode watch is rejected (eligibility).
void test_train_batch_two_members(Fixture& f) {
    const auto in_flight_before = f.committer.in_flight_requests();
    const auto drained_before = f.committer.prefill_drained_requests();
    const auto count_before = f.committer.counters().graph_batch_count;
    const auto nodes_before = f.committer.counters().total_nodes;
    const auto watches_before = f.committer.counters().total_watches;
    const auto registry_before = f.registry.size();
    const auto issue_calls_before = f.issue_calls;
    ...
    {
        auto expect_in_flight = in_flight_before;
        expect_in_flight.insert("req_A");
        expect_in_flight.insert("req_B");
        expect(f.committer.in_flight_requests() == expect_in_flight,
               "I: both train members in-flight after epoch 1");
        auto expect_drained = drained_before;
        expect_drained.insert("req_A");
        expect(f.committer.prefill_drained_requests() == expect_drained,
               "I: req_A prefill-drained after epoch 1");
        expect(f.committer.counters().graph_batch_count == count_before + 1,
               "I: setup epoch bumps graph_batch_count by exactly 1");
    }
    ...
    const auto& c = f.committer.counters();
    expect(c.graph_batch_count == count_before + 2,
           "I: graph_batch_count bumped by the train batch");
    expect(c.total_nodes == nodes_before + 12,
           "I: total_nodes gained the 12 train nodes");
    expect(c.total_watches == watches_before + 2,
           "I: total_watches gained the 2 member watches");
    {
        auto expect_in_flight = in_flight_before;
        expect_in_flight.insert("req_A");
        expect_in_flight.insert("req_B");
        ...
        auto expect_drained = drained_before;
        expect_drained.insert("req_A");
        expect_drained.insert("req_B");
        expect(f.committer.prefill_drained_requests() == expect_drained,
               "I: req_B drained by the train epoch's own delta fact");
    }
    expect(f.registry.size() == registry_before + 2,
           "I: registry holds the 2 member watches");
    {
        // Issue pass: the train's appended issue calls stay within the
        // touched ranks and cover all three (snapshot-relative -- the
        // wscllm part lineage differs from the mother's).
        std::vector<int> suffix(
            f.issue_calls.begin() + static_cast<std::ptrdiff_t>(
                                        issue_calls_before.size()),
            f.issue_calls.end());
        const std::set<int> suffix_ranks(suffix.begin(), suffix.end());
        expect(suffix_ranks == std::set<int>({0, 1, 2}),
...
// ----------------------------------------------------------- Part J ------
// Variant (sh_1.0 母本 Part I 对应; committer 机制层能力件): ONE train
// folding a prefill chunk of req_C (drain marker, prefill watch, gen 0)
// and a decode token of req_D (exit marker, decode watch, gen 1) -- the
// mixed train. Both watches fire. (wscllm 策略层按 §3.6 只发纯 decode
// 列车; 本件钉住 committer 对混合形态的机制能力, 与母本同构。)
void test_train_batch_drain_and_exit_markers(Fixture& f) {
    ...
    expect(f.committer.in_flight_requests().count("req_C") == 1 &&
               f.committer.in_flight_requests().count("req_D") == 1,
           "J: mixed-train members in-flight");
    expect(f.committer.prefill_drained_requests().count("req_D") == 1,
