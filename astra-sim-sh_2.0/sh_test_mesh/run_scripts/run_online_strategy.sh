#!/usr/bin/env bash
# sh_2.0 phase-1 step-1-10 官方在线 runner:strategy 模式(真实策略,实时物理)。
# Usage: bash run_online_strategy.sh <run_dir> <request_csv>
# request-neutral(裸仓库):仓库不预置输入队列;request_csv 由调用方按
# traces/materialize_first_30s.py 物化后必填传入(缺失即 fail-closed)。
...
# input-open 死端分支，对齐 wscllm 合同 §2.1；缺省关），不在本变量职责内。
...
# --request-window-rows 缺省 128）。四仓对齐 wscllm（sync-A16 批次4）：本仓
# 窗口为 advisory（calendar reader 按 arrival 序提交，窗口值不改变任何行为；
# 无 C++ 启动 span 预检——不设 A1 拒绝门，合同 §3.1），0/正数照原 token 透传
...
if [[ -n "${SH_METRICS_DETAIL:-}" ]]; then
  ...
  if [[ "${DETAIL}" != "off" && "${DETAIL}" != "summary" && "${DETAIL}" != "full" ]]; then
    echo "[run_online_strategy] SH_METRICS_DETAIL must be off|summary|full, got '${DETAIL}'" >&2
    exit 1
  fi
...
echo "[run_online_strategy] metrics detail=${DETAIL} (source: ${DETAIL_SOURCE})"
...
# A1/C1/D3(2026-08-28)三个降耗开关(runner 默认值,env 可覆盖):
#   --online-node-gc   默认 1(摊销化 NodeStore GC,含 store_ids prune);
...
ARCHIVED=0
for j in request_journal online_decision_log graph_batch_digests train_ledger ledger online_stats profile sensing_query_log; do
...
echo "[run_online_strategy] artifacts: ${ARCHIVED} jsonl archived -> results/; checkpoints=${CP_COUNT}; request_journal=results/request_journal.jsonl"
