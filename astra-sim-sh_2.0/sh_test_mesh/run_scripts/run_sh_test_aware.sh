...
echo "[sh_test] Scenario: ${MODEL_NAME} (${MLP_VARIANT}), ${HARDWARE_LABEL}, FACE mapping on ${NPUS_COUNT} NPUs in a ${MESH_SHAPE} mesh with ${INSTANCE_COUNT} unified TP=${TP_DEGREE} instances; p_chunk=${PREFILL_CHUNK_SIZE}; trace granularity=${TRACE_GRANULARITY}; Prefill uses resident-prefix affinity for partial sessions, Decode keeps the weighted Instance_map and minimum per-die LUT delta, while node-local HBM evicts FIFO session suffix halves before full-session fallback and shares HBM 50/50 between suffix restore and inference only while both are active; ${REQUEST_COUNT} requests across ${SESSION_COUNT} sessions, prefill ${PREFILL_RANGE}, decode ${DECODE_RANGE}."
"${ASTRA_SIM}" \
...
if command -v rg >/dev/null 2>&1; then
  CRITICAL_LOG_SEARCH=(rg -q '\[critical\]')
else
  CRITICAL_LOG_SEARCH=(grep -q '\[critical\]')
fi

if "${CRITICAL_LOG_SEARCH[@]}" "${RESULT_LOG}"; then
  echo "[sh_test] ASTRA-sim reported critical errors; see: ${RESULT_LOG}" >&2
  exit 1
fi
