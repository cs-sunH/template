#!/usr/bin/env bash
# run_online_wakeup_guard_fixture.sh -- 缺陷 C 回归 fixture(sh_2.0 同步版,
# 2026-08-16;源自 face 0049ef5 / face-defectfix2-done,face主动测试错误分析.md
# 缺陷 C:空队列分支唤醒条件)。
#
...
#
# 场景 2 defer-dead-end(唤醒真空死端;sh_2.0 与实录 bug#11 livelock guard
#   同族,已收敛为统一 dead-end guard):
#   注入 r0 @T -> 空批 defer(active=1、队列空、mailbox 空)-> 注入
#   CloseInput ->
#     EXPECT_MODE=fixed(缺省):C++ 立即 fail-closed abort,cpp.log 含
#       "lost-wakeup dead end" 且 active=1 计数入消息(替代忙等/静默
#       挂死),并保留 sh_2.0 的 per-rank waits-for 诊断转储;
...
RUN_ROOT=${1:-/tmp/sh20_wakeup_guard}

# Backport 同款(backport-fix1 5a):generated 目录名编码本窗口
# sess/req/p/d 区间与 trace-config digest,glob 动态解析唯一目录。
GEN_MATCH=("${PROJECT}"/sh_test_mesh/generated/llama2_7b_inference_54npus_*)
if [[ ${#GEN_MATCH[@]} -ne 1 || ! -d "${GEN_MATCH[0]}" ]]; then
  echo "[wakeup_guard] expected exactly one generated dir under sh_test_mesh/generated, found: ${GEN_MATCH[*]}" >&2
  exit 1
fi
ET_DIR=${GEN_MATCH[0]}
...
# sh_2.0 统一 guard 另含 run-end livelock 与 per-rank waits-for 转储)。
...
grep -q "run-end livelock" "${run_dir}/cpp.log" || {
  echo "[wakeup_guard] FAIL(场景2): 统一 guard 缺 sh_2.0 livelock 诊断行" >&2
  tail -12 "${run_dir}/cpp.log" >&2; exit 1; }
