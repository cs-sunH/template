#!/usr/bin/env python3
"""test_train_machinery.py -- sh_2.0 拼 batch 列车机制层 A1 夹具
（2026-08-22；§7.7 最小测试集在调度器机制层的合成覆盖，不跑仿真；
母本 sh_1.0 定型版同构，成员键 = request_index——sh_2.0 账本口径）。
...
def _runtime(request_id, *, ctx=100, decode=4, remaining_chunks=0,
             prefill_work=512, history=0, instance=0, queue_index=0):
    """face_scheduler._RequestRuntime 的测试替身（只读账本字段子集；
    列车推进字段与调度器 __init__ 的实例属性扩展同款）。"""
    rt = SimpleNamespace(
        ...
        decode_steps_remaining=decode,
        current_decode_token=ctx,
        history_location_before=None,
        decode_tokens_consumed=0,
        prefill_tokens_completed=0,
        drain_block_ends=None,
    )
...
def _bind(s, *runtimes):
    """把 runtime 挂进调度器账本（runtimes 列表 + 两个索引），返回
    各自 request_index（位次）。"""
    for runtime in runtimes:
        s._runtime_index[runtime.request.request_id] = len(s.runtimes)
        s.runtime_by_request_id[runtime.request.request_id] = runtime
        s.runtimes.append(runtime)
    return [s._runtime_index[runtime.request.request_id]
            for runtime in runtimes]
...
class TrainPlanningTest(unittest.TestCase):
    ...
    def test_uneven_decode_members_exit_in_one_train(self):
        ...
        for rid, remaining in (("ra", 5), ("rb", 9)):
            ...
            s._runtime_index[rid] = len(s.runtimes)
            s.runtime_by_request_id[rid] = rt
            s.runtimes.append(rt)
            state.active_decode.append(s._runtime_index[rid])
        plan = s._plan_train(state)
        self.assertEqual(plan["iterations"], 9)
        self.assertEqual(plan["exit_members"], [s._runtime_index["ra"],
                                                s._runtime_index["rb"]])
        self.assertEqual([p for _, p in plan["members"]], [5, 9])
        ...
        self.assertIsNone(plan["partial_first_chunk_count"])
    ...
    def test_mixed_train_chunk_per_iteration_and_drain_boundary(self):
        ...
        head_idx, member_idx = _bind(s, head, member)
        state.qp.append(head_idx)
        state.active_decode.append(member_idx)
        plan = s._plan_train(state)
        self.assertEqual(plan["iterations"], 3)
        self.assertEqual(plan["drain_members"], [head_idx])
        self.assertTrue(plan["head_first_chunk"])
        self.assertEqual([p for _, p in plan["members"]], [3])
    ...
    def test_pure_prefill_train(self):
        ...
        s = _bare_scheduler()
        state = s.instances[0]
        rt = _runtime("rp", ctx=512, decode=1, remaining_chunks=2,
                      prefill_work=1024)
        idx, = _bind(s, rt)
        state.qp.append(idx)
        plan = s._plan_train(state)
        self.assertEqual(plan["iterations"], 2)
        self.assertEqual(plan["drain_members"], [idx])
        self.assertEqual(plan["members"], [])
        self.assertEqual(plan["pass_spans"], [(512, 512), (512, 1024)])

    def test_partial_head_train_span_layout(self):
        """sh_2.0 特性保留：partial 前缀两段式迁移的队列头——span 平铺
        为 [首 chunk] + [各成员第 1 迭代] + [其余 chunk + 成员剩余]，
        partial_first_chunk_count = 1 + 成员数（层段拆分组界）。"""
        s = _bare_scheduler()
        state = s.instances[0]
        head = _runtime("rh", ctx=0, decode=1, remaining_chunks=2,
                        prefill_work=1024, history=0)
        head.history_location_before = SimpleNamespace(
            location="partial_hbm_remote", instance_index=1,
            resident_prefix_layers=6)
        member = _runtime("rm", ctx=100, decode=3)
        head_idx, member_idx = _bind(s, head, member)
        state.qp.append(head_idx)
        state.active_decode.append(member_idx)
        plan = s._plan_train(state)
        self.assertTrue(plan["partial"])
        self.assertEqual(plan["partial_first_chunk_count"], 2)  # 1 + 成员
        # 成员 decode=3、列车 2 迭代 ⇒ participation=2（跨列车存续）。
        self.assertEqual(
            plan["pass_spans"],
            [(512, 512), (1, 101), (512, 1024), (1, 102)])
        # 非 partial 头（local_hbm）不触发拆分组。
        head.history_location_before = SimpleNamespace(
            location="local_hbm", instance_index=1,
            resident_prefix_layers=6)
        state.train_seq = 0
        plan2 = s._plan_train(state)
        self.assertFalse(plan2["partial"])
        self.assertIsNone(plan2["partial_first_chunk_count"])
...
class TrainFinalizeTest(unittest.TestCase):
    ...
    def test_advance_is_closed_form_exact(self):
        """闭式账本：列车核销后 decode_tokens_consumed/current_decode_token
        与逐 token 精确值逐点一致；退出成员移出 active_decode。"""
        s = _bare_scheduler()
        state = s.instances[0]
        rt = _runtime("rm", ctx=100, decode=9)
        idx, = _bind(s, rt)
        state.active_decode.append(idx)
        state.in_flight_train = {
            "train_id": "t1", "iterations": 4, "members": [(idx, 4)],
            "exit_set": {idx}, "drain_set": set(),
            "signal_set": {idx},
        ...
        self.assertEqual(rt.decode_steps_remaining, 5)
    ...
    def test_same_tick_multi_instance_and_split_delivery_idempotent(self):
        ...
        x_idx, d_idx = _bind(s, rt_x, rt_d)
        st0.active_decode.append(x_idx)
        st0.qp.append(d_idx)
        st0.in_flight_train = {
            "train_id": "t0", "iterations": 3,
            "members": [(x_idx, 3)],
            "exit_set": {x_idx}, "drain_set": {d_idx},
            "signal_set": {d_idx, x_idx},
            "prefill_chunk_tokens": [(d_idx, 512), (d_idx, 512)],
        }
        # 实例 1：同 tick 完成的另一列车。
        st1 = s.instances[1]
        rt_y = _runtime("my", ctx=50, decode=2, instance=1)
        y_idx, = _bind(s, rt_y)
        st1.active_decode.append(y_idx)
        st1.in_flight_train = {
            "train_id": "t1", "iterations": 2,
            "members": [(y_idx, 2)],
            "exit_set": {y_idx}, "drain_set": set(),
            "signal_set": {y_idx},
            "prefill_chunk_tokens": [],
        }
        ...
        self.assertEqual(list(st0.qp), [d_idx])  # drain 事件未达，仍在 qp
