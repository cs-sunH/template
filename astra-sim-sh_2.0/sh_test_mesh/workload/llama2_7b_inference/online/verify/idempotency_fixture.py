...
一个真实的 Sh20OnlineScheduler,每个 delta 喂两次:
...
即删的 has_any_response 判定),按本仓适配三处(其余与 face 版逐字一致):
  ...
  1. 调度器装配段:Sh20OnlineScheduler(与本仓 online_service.py 相同的
     关键字装配,calibrated_constants 走调度器缺省推导);
  2. _snapshot 的 KV 口径:face 版读 kv_manager.events 事件流计数 +
     scheduler._kv_events_emitted 水位(本仓 KVCacheManager 无事件流
     API,调度器亦无该水位;KV 动作事实由 _kv_action_rows 从决策产物
     取入 kv_actions 批行),改用等价状态投影——逐会话
     session_snapshots()(含本仓三态账本的 resident_prefix_layers)+
     逐 rank hbm_snapshots() 的 kv_cache_bytes(形态对齐批 F1 sh_1.0 /
     批 F2a sh_3.0 同名夹具);
  3. --config 传参:与本仓 online_service.py 相同,args.config(str)统一
     转 Path 再进 load_face_trace_config(裸传 str 会在
     load_config_rows 的 .exists() 处崩溃)。
"""
...
def _snapshot(scheduler, sink):
    ...
    return {
        ...
        # sh_2.0 适配(批 F2b 2026-08-21,形态对齐批 F1 sh_1.0 / 批 F2a
        # sh_3.0):face 版此处读 kv 事件水位(kv_manager.events 长度 +
        # _kv_events_emitted);本仓 KVCacheManager 无事件流水(KV 动作事实
        # 由 _kv_action_rows 从决策产物取入 kv_actions 批行,账本键见下),
        # 等价幂等观测源 = 逐会话 KV 状态(位置/实例/上下文/字节/驻留前缀
        # 层数/完成时刻/active)+ 逐 rank HBM KV 占用——任何重放误重入账
        # 都会改变其中至少一项(reserve/prepare/expand/move/enforce/
        # mark_complete 均落到会话或 rank 字节;重放误重跑策略会先撞基类
        # fail-closed)。
        "kv_sessions": [
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        description="sh_2.0 phase-4 §7.2 idempotency fixture")
    ...
    if args.config:
        ...
        # 为 Path),裸传 str 会在 load_config_rows 的 .exists() 处崩溃——
        # 与本仓 online_service.py 相同,统一转 Path。
