...
planner memory deltas recorded by ``face_scheduler.KVCacheManager`` through
the read-only :class:`metrics_schema.MemoryMetricsObserver`, and finally
writes the ``metrics_manifest.json`` sidecar next to the generated ``.et``
files.
...
class PlannerRooflineStatsAccumulator:
    ...
    count/sum/min/max are kept, so memory stays O(cells) regardless of
    iteration count. Cells are keyed by ``(phase, tp_degree, batch,
    kv_bin)`` where phase is ``prefill`` / ``decode`` / ``mixed`` (a mixed
    iteration carries both a prefill chunk and a decode batch), batch is the
    prefill chunk for prefill cells and the decode batch otherwise, and
    kv_bin is the power-of-two ceiling of the decode KV length.
    """
...
def kv_event_payload_sh2(request_records: Sequence[Mapping[str, Any]]) -> list[list[Any]]:
    ...
    rows: list[list[Any]] = []
    for record in sorted(request_records, key=lambda item: int(item["queue_index"])):
        for transfer in record.get("kv_transfers", []):
            rows.append([record["queue_index"], transfer])
    return rows
...
class MemoryActionRecorder:
    ...
    def __init__(self, observer: MemoryMetricsObserver) -> None:
        ...
        self.current_planner_time_ns = 0

    def set_planner_time(self, now_ns: int) -> None:
        """Stamp the planner event-loop time used by subsequent records."""

        if isinstance(now_ns, bool) or not isinstance(now_ns, int) or now_ns < 0:
            raise ValueError("planner time must be a non-negative integer")
        self.current_planner_time_ns = now_ns
    ...
    def record(
        self,
        *,
        planner_time_ns: Optional[int] = None,
        anchor_kind: str,
        ...
        if planner_time_ns is None:
            planner_time_ns = self.current_planner_time_ns
        delta = MemoryDelta(
