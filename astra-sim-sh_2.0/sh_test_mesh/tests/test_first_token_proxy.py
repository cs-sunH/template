#!/usr/bin/env python3
"""B4/WP9 fallback: train-interpolated first-token proxy unit tests (S2).

Covers the B3_S2 60s gate-2 failure fallback implemented in
workload/llama2_7b_inference/metrics_postprocess.py:
  ...
  2. the proxy formula hand-check on a synthetic train ledger index
     (sum domain = train iterations N, clamp, last-train / no-joiner NA);
...
class ModelBytesTests(unittest.TestCase):
    def test_authoritative_conversions(self):
        ...
        # test_face_scheduler.py:404 freezes estimate_model_weight_bytes at
...
class ProxyFormulaTests(unittest.TestCase):
    ...
    def test_short_debut_interpolates_within_train(self):
        ...
        the debut exits before the train boundary.  decode_length==1 is
        no longer interpolated but pinned to completion -- see
...
class EndToEndRowsTests(unittest.TestCase):
    ...
    def _rows(self):
        ...
        _, service_manifest, _ = mp._load_manifest_sidecars(runs[0])
        rows, _summary = mp._request_metric_rows(runs[0], service_manifest)
    ...
    def test_proxy_filled_from_ledger_and_tagged(self):
        ...
        expected = int(round(
            1_000_000 + (weights[0] / sum(weights)) * 8_000_000))
        self.assertEqual(row["first_token_ns"], str(expected))
    ...
    def test_dl1_debut_pinned_to_completion(self):
        ...
        and the dl1 hard invariant in _request_metric_rows does not trip."""
        ...
        self.assertEqual(row["completion_ns"], "41000000")
        self.assertEqual(row["first_token_source"], "train_interpolated")
        self.assertIn("pinned_to_completion", row["instructions"])

    def test_summary_counts_first_token_sources(self):
        self._write_log()
        with (self.results_dir / "train_ledger.jsonl").open(
                "w", encoding="utf-8") as sink:
            sink.write(json.dumps(_ledger_row(
        ...
        runs = mp._parse_logs([self.log_path])
        _, service_manifest, _ = mp._load_manifest_sidecars(runs[0])
        _rows, summary = mp._request_metric_rows(runs[0], service_manifest)
        self.assertEqual(summary["first_token_source_counts"],
                         {"exact": 1, "train_interpolated": 1})
