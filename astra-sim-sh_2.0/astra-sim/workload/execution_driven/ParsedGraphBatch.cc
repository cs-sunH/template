...
ParsedNode parse_node(const nlohmann::json& node_json, size_t index,
    ...
    // N2: the frozen 13-key set plus the sh_2.0-specific optional node key
    // is_local_hbm_kv_restore (§4.3.3 MEM_LOAD restore-DMA routing bit,
    // emitted only when true; absent == false).
    check_key_set(
        node_json,
        {"rank", "id", "name", "type", "is_cpu_op", "is_timer_op",
         "inputs_values", "request_id", "stage", "generation", "compute",
         "comm", "coll", "is_local_hbm_kv_restore"},
        {"rank", "id", "name", "type", "is_cpu_op", "is_timer_op",
         "inputs_values", "request_id", "stage", "generation", "compute",
         "comm", "coll"},
        where);
    ...
    if (node_json.contains("is_local_hbm_kv_restore")) {
        node.is_local_hbm_kv_restore = get_bool(
            node_json["is_local_hbm_kv_restore"], "is_local_hbm_kv_restore",
            where);
    }
    node.inputs_values =
    ...
    // {remote_weight_bytes, hbm_access_mode}.
    ...
    if (compute.contains("hbm_access_mode")) {
        // sh_2.0 adaptation: the OnlineNode carries the N-way HBM contention
        // charging mode as a TOP-LEVEL field (not inside ComputeAttrs).
        node.hbm_access_mode =
            get_int(compute["hbm_access_mode"], "hbm_access_mode",
                    where + ".compute");
    }
    ...
    if (comm.contains("hbm_charge")) {
        // sh_2.0 adaptation: the p2p endpoint charging switch is a TOP-LEVEL
        // OnlineNode field (not inside CommAttrs); absent == the struct
        // default true, identical to the old value(key, true) read.
        node.hbm_charge =
