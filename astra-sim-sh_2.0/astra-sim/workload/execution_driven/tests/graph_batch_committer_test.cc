...
future arrival) through:
  ...
  Part H  Online HBM key unification nails (低-2): the snake_case section
          keys comm.hbm_charge / compute.hbm_access_mode are parsed into
          the TOP-LEVEL NodeView fields (the struct layout is unchanged);
          [C1 note] the malformed-value rejects moved to the parse layer
          (non-boolean comm.hbm_charge throws ParseError before any
          committer state exists); the negative compute.hbm_access_mode
          DOMAIN rule stays in validate(); the legacy top-level kebab keys
          node["hbm-charge"] / node["hbm-access-mode"] are now unknown
          node keys -> parse-rejected fail-closed (absent keys keep the
          struct defaults, same observable outcome as the old inert read).
  Part I  拼 batch §3.1 前置验证, 2026-08-22 (母本 sh_1.0 Part H 同构;
          sh_2.0 的字母 H 已被 HBM key nails 占用): a two-request decode
          train (shared aggregate body nodes under the batch-namespace
          ...
          rejected with zero side effects. Exercises the sh_2.0
          node-coverage exemption: the batch-namespace body/barrier nodes
          bypass the node-only stage-boundary eligibility (they carry no
          request).
  Part J  拼 batch §3.1 前置验证, 2026-08-22 (母本 Part I 同构; variant):
          one train carrying BOTH a drain marker (prefill watch, gen 0)
          and an exit marker (decode watch, gen 1) of two DIFFERENT
...
Run (from template/astra-sim-sh_2.0):
...
// NodeView fields (the struct layout is unchanged). C1 (2026-08-29): the
// batches go through the production parse -- a non-boolean comm.hbm_charge
// throws ParseError BEFORE any committer state exists (the JSON-shape half
// moved to the parse layer); the negative compute.hbm_access_mode DOMAIN
// rule stays in validate() (typed read); the legacy top-level kebab keys
// node["hbm-charge"] / node["hbm-access-mode"] are unknown node keys now
// and fail-closed at the parse (their absence keeps the struct defaults --
// the same observable outcome the old inert read produced).
void test_hbm_key_parsing(Fixture& f) {
    ...
    const auto hbm_response = [] {
        ...
        recv["request_id"] = "r10";
        // pre-C1 the legacy top-level kebab key recv["hbm-charge"] sat
        // inert here; C1's frozen node key set rejects it fail-closed, so
        // the nail drops the key (absent == the struct default true).
        nlohmann::json resp = empty_response(4);
        resp["nodes"] = nlohmann::json::array({comp, send, recv});
        resp["watches"] = nlohmann::json::array({
    ...
    {
        // JSON-shape half: non-boolean comm.hbm_charge -> ParseError, zero
        // committer-state impact (parse is a pure local construction).
        const Snapshot before = snapshot_of(f);
        nlohmann::json resp = hbm_response();
        resp["nodes"][1]["comm"]["hbm_charge"] = "x";
        bool threw = false;
        try {
            (void)parse_batch(resp);
        } catch (const ParseError&) {
            threw = true;
        }
        expect(threw, "H: comm.hbm_charge non-boolean rejected (ParseError)");
        expect(snapshot_of(f) == before,
               "H: the parse rejection left the committer state untouched");
    }
    {
        // Domain half: negative compute.hbm_access_mode parses (int32) and
        // is rejected by validate()'s typed domain rule.
        nlohmann::json resp = hbm_response();
    ...
    const GraphBatch b = parse_batch(hbm_response());
    ...
    expect(recv_view.has_value() && recv_view->hbm_charge &&
           ...
           "H: absent hbm keys keep the defaults (true / 0)");
}

// ------------------------------------- 拼 batch §3.1 前置验证, 2026-08-22 ----
// Frozen batch-train schema (拼 batch 改造;母本 sh_1.0 定型版同款):
...
// The batch-namespace body/barrier nodes exercise the sh_2.0 node-coverage
// exemption (拼 batch 适配: "batch_train_" prefix bypasses the node-only
// stage-boundary eligibility).
void test_train_batch_two_members(Fixture& f) {
    ...
    expect(f.committer.counters().graph_batch_count == 6,
           "I: graph_batch_count == 6 after the setup epoch");
    ...
    expect(c.total_nodes == 25, "I: total_nodes == 25 (13 + 12)");
    expect(c.max_nodes_per_batch == 12, "I: max_nodes_per_batch == 12");
    expect(c.total_watches == 6, "I: total_watches == 6 (4 + 2 members)");
    expect(f.committer.in_flight_requests() ==
               std::set<std::string>({"r3", "r9", "r10", "req_A", "req_B"}),
    ...
    expect(f.registry.size() == 6,
           "I: registry holds the 2 member watches (+4 legacy)");
    expect(f.issue_calls ==
               std::vector<int>({0, 1, 2, 0, 0, 0, 1, 0, 1, 2}),
           "I: issue pass covered exactly the touched ranks {0, 1, 2}");
    // Per-rank pending nodes after Part H: {7, 4, 2}; the train adds 4.
    const size_t pending_after_h[3] = {7, 4, 2};
    for (int rank = 0; rank < 3; ++rank) {
        expect(f.sources[rank]->store().pending_count() ==
                   pending_after_h[rank] + 4,
...
// ----------------------------------------------------------- Part J ------
// Variant (母本 sh_1.0 Part I 同构): ONE train folding a prefill chunk of req_C (drain marker,
...
void test_train_batch_drain_and_exit_markers(Fixture& f) {
    ...
    const auto& c = f.committer.counters();
    expect(c.graph_batch_count == 8, "J: graph_batch_count == 8");
    expect(c.total_nodes == 37, "J: total_nodes == 37 (25 + 12)");
    expect(c.total_watches == 8, "J: total_watches == 8 (6 + 2)");
    expect(f.committer.in_flight_requests() ==
               std::set<std::string>(
    ...
    expect(f.registry.size() == 8, "J: registry holds both member watches");
