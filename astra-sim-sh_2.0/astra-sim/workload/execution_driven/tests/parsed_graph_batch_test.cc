...
void test_max_shape_round_trip() {
    ...
    // (hbm_access_mode / hbm_charge), not inside ComputeAttrs/CommAttrs.
    ...
    expect(comm.bytes == 128 && comm.src == 0 && comm.dst == 1 &&
               comm.tag == 7 && parsed.nodes[0].node.hbm_charge == false,
    ...
    expect(parsed.nodes[1].node.is_local_hbm_kv_restore == true,
           "P: present is_local_hbm_kv_restore round-trips true");
