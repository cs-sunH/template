...
struct OnlineNode {
    ...
    // sh_2.0/sh_3.0 specific (§4.3.3): MEM_LOAD/MEM_STORE nodes carrying this
    ...
    // lookup entirely. Placed at the tail of this sh_2.0 bool group to
    // reuse its trailing padding (the variant fields end at offset 64 and
    // the next member is 8-byte aligned at 72), keeping sizeof(OnlineNode)
    // unchanged (locked by the R2 regression's static_asserts). Static-mode
    // views built by ETFeederGraphSource leave both flags false.
