...
main_online.cc -- online (execution-driven) entry point (sh_2.0; wscllm phase-1 blueprint).
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        // Defect fix A (2026-08-16, face主动测试错误分析.md 缺陷 A; 同步自
        // face 0049ef5 / face-defectfix2-done): pump() QUEUES Submit commands
        // into the bounded ingress queue; they only register as pending
        // alarms (on_alarm_scheduled) on the NEXT drain_commands(). Between
        // this pump() and that drain the service counters pass through a
        // vacuum -- active==0 && pending_alarm==0 while queued-but-undrained
        // commands exist -- and the finished() check below could end the run
        // with thousands of CSV rows still undelivered (F2/F3/F4:
        // ack_count != delivery_count, 恒差 1; the window had topped up
        // un-consumed rows right as the last pending alarm of the previous
        // batch fired). Drain whatever pump() just queued BEFORE consulting
        // finished(), so the check can never observe the vacuum. The extra
        // drain is a no-op when pump() queued nothing (the common case).
        ...
        if (event_queue->finished()) {
            ...
            if (mailbox.has_decision_work()) {
                ...
                // 同步自 face 0049ef5): the main list is empty but the
                // same-tick deferred queue still holds unexecuted events
                // (scheduled outside a proceed context -- e.g. a deferred
                // issue pass appended after the drain pass ended).
                // finished() is blind to them and no proceed() would ever
                // run to drain them: force the explicit next decision
                // boundary (same mechanism as the milestone branch above)
                // so a proceed() executes and drains the deferred queue.
                // NOT a delivery deferral (no mailbox work):
    ...
    // the run; retained = records still in stores at run end (the in-flight
    // window). A1: the forced final drain right above leaves only
