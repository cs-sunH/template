...
* **face/wscllm**：双级（-LRU 方案 A，2026-09-07）：新产物优先
  ...
  partial_hbm_remote→partial / remote_memory→full；evidence 列区分
  full_local/full_remote），逐出/恢复消费契约行（history_transfers
  逐段对象 + history_evictions/prefill_evictions/decode_evictions/
  completion_eviction_transfers；B3 双序列化的 legacy admission/
...
* **逐仓覆盖度**（decision-log 重放路径的口径；B0 基线 60s_summary 实测）：
  ...
  * face（-LRU，B4 2026-09-07）：同一触发字段（history_transfers 在场）
    升级 full 口径——契约逐出行替换 legacy 列表（防双计）、restore 逐段
    对账（tiered 标量聚合把 partial 前缀 noc move 段与 suffix remote
    add 段混计，会击穿占用下界）、completion 契约行 =
    completion_eviction_transfers；decode 的 prefill_decode_transfer
...
* **fail-closed**：缺 token manifest/trace_config、tick 回退、请求集两源
  ...
  中断。
* **测试**：`tests/test_hbm_watermark.py`（38 例：手算桶时序/峰值/均值/
  逐出/违规计数、四仓语义各一例、face-tiered 升级两例（B4：契约行防
  双计 + PD 列表行回归钉）、fail-closed 六例、列序冻结、四层 tier
