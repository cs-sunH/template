...
class FaceSchedulerTests(unittest.TestCase):
    ...
    def test_hardware_remote_memory_expansion_parses(self) -> None:
        """B1 A.2 reversal (2026-09-06): the remote memory backend was
        restored from sh_2.0; a hardware config declaring PER_NPU memory
        expansion parses legally with its remote-memory fields resolved."""
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            hardware = load_hardware_config(source, "validation-160gib")
        self.assertEqual(hardware.remote_memory_type,
    ...
    def test_kv_cache_policy_accepts_session_lru_tiered(self) -> None:
        ...
        def edit_policy(value: str):
            def edit(line: str):
                if line.startswith("config,kv_cache_policy,"):
                    return f"config,kv_cache_policy,{value},,,,B2 value domain\n"
                return line
            return edit
    ...
    def test_exact_tp_shards_and_session_lru_recompute(self) -> None:
        ...
        # B2:三态快照——完成会话驻留本实例(local_hbm),local=全层字节
        # (全 rank 求和 = 160B/rank × 2 rank)。
        self.assertEqual(snapshot_a.location, "local_hbm")
        self.assertEqual(snapshot_a.resident_prefix_layers, model.layers)
        self.assertEqual(snapshot_a.local_bytes, 320)
        self.assertEqual(snapshot_a.remote_bytes, 0)
        self.assertEqual(snapshot_a.logical_context_tokens, 40)
        # 41 token 增量 4B/rank <= 余 8B/rank:同实例原地放行(LOCAL_HIT,
        # 零重算——三态形态下完成边界保留 KV,RECOMPUTE 已删除)。
    ...
    def test_manager_evicts_multiple_lru_victims_but_not_active_kv(self) -> None:
        ...
        # 逐出 = 整体外迁:会话 REMOTE、无本地实例、无驻留层,逻辑上下文保留。
        snapshot_a = manager.session_snapshot("a")
        self.assertEqual(snapshot_a.logical_context_tokens, 10)
        self.assertEqual(snapshot_a.location, "remote_memory")
        self.assertIsNone(snapshot_a.instance_index)
        self.assertEqual(snapshot_a.resident_prefix_layers, 0)
        self.assertEqual(snapshot_a.remote_bytes, 80)  # 40B/rank × 2 rank
        self.assertTrue(manager.grow_prefill("c", 30, 30, "c0").admitted)
        self.assertEqual(manager.session_snapshot("c").location, "local_hbm")
...
class PassiveEvictionModeTests(unittest.TestCase):
    ...
    def test_passive_completion_retains_kv_and_final_state_passes(self) -> None:
        ...
        # 完成边界零逐出——retain_complete 事件在场,会话 LOCAL 驻留。
        event_types = [
    ...
    def test_passive_admission_evicts_exactly_enough(self) -> None:
        ...
        self.assertEqual(decision.evictions[0].transfer.kind, "remote_store")
        self.assertEqual(decision.evictions[0].transfer.layer_start, 0)
