...
bool commit_gc_batch(GcFixture& fixture, const GraphBatch& batch,
    ...
    // sh_2.0 node-only segment gate: a prefill (request, stage) whose watch
    // is not in this batch is admitted only when the request is in-flight --
    // synthesize the ARRIVAL fact for fixture requests the committer has
    // not seen yet (the real service always delivers arrivals first).
