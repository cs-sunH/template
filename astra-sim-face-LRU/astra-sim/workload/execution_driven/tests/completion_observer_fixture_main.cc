...
4-node-per-rank graph (plus the remote-mem node on the edge ranks)
generated by tests/make_completion_fixture_et.py:
...
the hook count equals the number of completed nodes (npus * 4 + 26 = 242
at 54 ranks: the sh_2.0 self-driven chains add the LocalHbmBandwidthModel
restore DMA node on every rank and the AnalyticalRemoteMemory FIFO node on
the 26 edge ranks with a configured port).
...
struct FixtureCounters {
    ...
    uint64_t success = 0;
    uint64_t skipped = 0;
    uint64_t site_skip = 0;     // node id 0 (skip_invalid)
    uint64_t site_generic = 0;  // node id 1 (generic wlhd branch) + id 4 (hbm)
    uint64_t site_coll = 0;     // node id 2 (collective branch)
    uint64_t site_remote = 0;   // node id 3 (remote-mem FIFO chain, edge ranks)
};
...
void counting_hook(void* ctx, int /*rank*/, uint64_t node_id,
    ...
    switch (node_id) {
        case 0:
            ...
            break;
        case 1:
        case 4:  // HBM restore DMA chain completes via the wlhd branch
            counters->site_generic++;
            break;
        case 3:  // AnalyticalRemoteMemory FIFO chain -> wlhd branch
            counters->site_remote++;
            break;
        default:
            counters->site_coll++;
            break;
...
int main(int argc, char* argv[]) {
    ...
    //    (npus * 4 + 26 = 242 at 54 ranks; incl. remote-mem and HBM
    //    restore chains);
    //  - all three terminal insertion sites fired (skip/coll 54 each,
    //    generic 108 incl. the HBM restore node, remote 26 edge ranks);
    ...
    if (counters.site_skip != npus_count ||
        counters.site_generic != 2 * npus_count ||
        counters.site_coll != npus_count ||
        counters.site_remote != 26) {
        std::fprintf(stderr,
                     "[fixture] FAIL: terminal sites (skip/generic/coll/"
                     "remote) expected %u/%u/%u/26, got %llu/%llu/%llu/%llu\n",
                     npus_count, 2 * npus_count, npus_count,
                     static_cast<unsigned long long>(counters.site_skip),
                     static_cast<unsigned long long>(counters.site_generic),
                     static_cast<unsigned long long>(counters.site_coll),
                     static_cast<unsigned long long>(counters.site_remote));
        ok = false;
    }
    if (counters.skipped != npus_count || counters.success != 3 * npus_count + 26) {
        std::fprintf(stderr,
                     "[fixture] FAIL: expected skipped=%llu success=%llu, "
                     "got %llu/%llu\n",
                     static_cast<unsigned long long>(npus_count),
                     static_cast<unsigned long long>(3 * npus_count + 26),
                     static_cast<unsigned long long>(counters.skipped),
                     static_cast<unsigned long long>(counters.success));
        ok = false;
    }
