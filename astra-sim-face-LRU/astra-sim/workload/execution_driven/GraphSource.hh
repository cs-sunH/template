...
struct OnlineNode {
    ...
    // next member is 8-byte aligned), keeping the pre-HBM layout
    // unchanged (locked by the R2 regression's static_asserts).
    // Static-mode views built by ETFeederGraphSource leave both flags
    // false.
    ...
    // sh_2.0 HBM fields (§4.3.3), placed at the struct tail: the N-way
    // contention model's MEM-node routing bit and pool endpoint charging
    // mode. Keeping them out of the head bool group preserves the R2
    // metric-anchor padding-reuse proof (see the metric flag comment above
    // and the mirror static_assert in metric_collector_one_shot_erase_test).
    // is_local_hbm_kv_restore: MEM_LOAD/MEM_STORE nodes carrying this
    // attribute dispatch to issue_local_hbm_kv_restore (LocalHbmBandwidthModel
    // restore DMA, HardwareResource hbm_dma class) instead of the remote-mem
    // FIFO; absent == false. hbm_access_mode (attr "hbm-access-mode"):
    // 0/absent = none, 1 = local HBM read, 2 = local HBM write, bytes =
    // tensor_size; the port transaction and the HBM job complete as a join
    // (Workload hbm_endpoint_joins_).
